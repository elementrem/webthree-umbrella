...
SendDialog::SendDialog(QWidget* _parent, AleleFace* _alele):
	QDialog(_parent),
...
{
	QString str("0xd57b6c60f48f4187edf735f627400bf03e5c7f02"); //just some elementrem address
...
{
	tie(m_to, m_data) = m_alele->readAddress(_s.toStdString());
	updateProblem();
}
...
void SendDialog::on_send_clicked()
{
	if (!m_to || m_value == Invalid256)
		...
		if (Secret sender = m_alele->retrieveSecret(m_alele->author()))
		{
			m_alele->elementrem()->submitTransaction(sender, m_value, m_to, m_data);
			accept();
		}
}
