...
const char* DappLoaderChannel::name() { return EleOrange "Ð->"; }
#define cdapp clog(DappLoaderChannel)

namespace dev { namespace alele { QString contentsOfQResource(std::string const& res); } }
...
{
	QUrl url(_uri);
	if (!url.scheme().isEmpty() && url.scheme() != "ele")
...
{
	/// https://github.com/elementrem/go-elementrem/wiki/URL-Scheme
	Manifest manifest;
