...
SlaveAux::SlaveAux():
	m_engine(make_shared<Elhash>()),
	m_id(h256::random())
{
	m_engine->farm().onSolutionFound([&](ElhashProofOfWork::Solution const& sol) { return onSolutionFound(sol); });
}
...
void SlaveAux::start()
{
	...
	{
		m_engine->farm().setWork(ElhashProofOfWork::WorkPackage());
...
void SlaveAux::timerEvent(QTimerEvent* _e)
{
	...
	if (_e->timerId() == m_rateTimer)
	{
		...
		try
		{
			rpc.ele_submitHashrate(toJS(m_hashrate), "0x" + m_id.hex());
		}
	...
	if (_e->timerId() == m_workTimer)
	{
		...
		try
		{
			...
			if (!v[0].asString().empty())
			{
				...
				if (!(m_dag = ElhashAux::full(newSeedHash, true, [&](unsigned _pc){ onGeneratingDAG(_pc); return 0; })))
				{
...
void SlaveAux::onNewWork(ele::ElhashProofOfWork::WorkPackage const& _work)
{
...
bool SlaveAux::onSolutionFound(ele::ElhashProofOfWork::Solution const& _sol)
{
...
bool SlaveAux::reportSolution(ele::ElhashProofOfWork::Solution const& _sol)
{
	try
	{
		if (ElhashAux::eval(m_current.seedHash, m_current.headerHash, _sol.nonce).value < m_current.boundary)
		{
			...
			cnote << "  Elhash: " << h256(ElhashAux::eval(m_current.seedHash, m_current.headerHash, _sol.nonce).value).hex();
			::FarmClient rpc(*m_client);
			bool ok = rpc.ele_submitWork("0x" + toString(_sol.nonce), "0x" + toString(m_current.headerHash), "0x" + toString(_sol.mixHash));
			if (ok)
	...
	m_aux(new SlaveAux)
{
	...
	qRegisterMetaType<dev::ele::ElhashProofOfWork::Solution>();
	qRegisterMetaType<dev::ele::ElhashProofOfWork::WorkPackage>();
	m_aux->moveToThread(&m_worker);
	connect(m_aux, SIGNAL(solutionFound(ele::ElhashProofOfWork::Solution)), SIGNAL(solutionFound(ele::ElhashProofOfWork::Solution)));
