...
ele::Client* AleleFace::elementrem() const
{
	return web3()->elementrem();
}

std::shared_ptr<shh::WhisperHost> AleleFace::whisper() const
{
	return web3()->whisper();
}

Address AleleFace::author() const
{
	return elementrem()->author();
}

void AleleFace::setAuthor(Address const& _a)
{
	cdebug << "Setting author" << _a;
	if (elementrem()->author() != _a)
	{
		elementrem()->setAuthor(_a);
		emit beneficiaryChanged();
	}
}

string AleleFace::toReadable(Address const& _a) const
{
	string p = toName(_a);
	string n;
...
pair<Address, bytes> AleleFace::readAddress(std::string const& _n) const
{
...
string AleleFace::toHTML(StateDiff const& _d) const
{
	stringstream s;
...
string AleleFace::dnsLookup(string const& _a) const
{
