...
{
	if ((m_alele && m_alele->isOpen() && m_alele->elementrem()->isSyncing()) || _e->timerId() == m_secondTimer)
		update();
}
...
void SyncView::paintEvent(QPaintEvent*)
{
	...
	if (!m_alele || !m_alele->isOpen() || !isVisible() || !rect().width() || !rect().height())
		return;

	auto client = m_alele->elementrem();
