...
public:
	explicit TransactDialog(QWidget* _parent, AleleFace* _alele);
	~TransactDialog();

	void resetGasPrice();
	void setEnvironment(dev::AddressHash const& _accounts, dev::ele::Client* _ele, NatSpecFace* _natSpecDB);
...
private slots:
	...
	void updateBounds();
	void finaliseBounds(dev::ele::ExecutionResult const& _er);

signals:
	void gasEstimationProgress();
	void gasEstimationComplete(dev::ele::ExecutionResult const& _er);
...
private:
	...
	AleleFace* alele() const { return m_alele; }
	dev::ele::Client* elementrem() const { return alele()->elementrem(); }
