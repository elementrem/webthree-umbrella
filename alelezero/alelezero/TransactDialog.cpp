...
#include <libelementrem/Utility.h>
...
TransactDialog::TransactDialog(QWidget* _parent, AleleFace* _alele):
	QDialog(_parent),
	...
	m_alele(_alele)
{
	...
	qRegisterMetaType<dev::ele::ExecutionResult>("dev::ele::ExecutionResult");
...
void TransactDialog::setEnvironment(AddressHash const& _accounts, dev::ele::Client*, NatSpecFace* _natSpecDB)
{
	...
	{
		u256 b = elementrem()->balanceAt(address, PendingBlock);
		QString s = QString("%2: %1").arg(formatBalance(b).c_str()).arg(QString::fromStdString(alele()->toReadable(address)));
		m_ui->from->addItem(s);
	}
	updateDestination();
...
{
	setValueUnits(m_ui->gasPriceUnits, m_ui->gasPrice, alele()->elementrem()->gasPricer()->bid());
}
...
void TransactDialog::updateDestination()
{
	...
	for (Address const& a: alele()->allKnownAddresses())
		in.insert(QString::fromStdString(alele()->toReadable(a)), QString::fromStdString(a.hex()));
...
//	ui->fee->setText(QString("(gas sub-total: %1)").arg(formatBalance(fee()).c_str()));
	...
	for (auto const& i: m_accounts)
		if (elementrem()->balanceAt(i) >= totalReq)
		{
			ok = true;
			break;
		}
...
void TransactDialog::on_destination_currentTextChanged(QString)
{
	if (m_ui->destination->currentText().size() && m_ui->destination->currentText() != "(Create Contract)")
	{
		auto p = toAccount();
		if (p.first)
			m_ui->calculatedName->setText(QString::fromStdString(alele()->toReadable(p.first)));
		else
...
{
	auto a = fromAccount();
	u256 nonce = m_ui->autoNonce->isChecked() ? elementrem()->countAt(a, PendingBlock) : m_ui->nonce->value();
	...
	else if (sourceIsSolidity(_user))
	{
		...
		try
		{
			...
			else
			{
				...
				solidity += "<pre>var " + compiler.defaultContractName() + " = web3.ele.contract(" + QString::fromStdString(compiler.interface()).replace(QRegExp("\\s"), "").toHtmlEscaped().toStdString() + ");</pre>";
...
pair<Address, bytes> TransactDialog::toAccount()
{
	...
	if (!isCreation())
	{
		if (!m_ui->destination->currentData().isNull() && m_ui->destination->currentText() == m_ui->destination->itemText(m_ui->destination->currentIndex()))
			...
			p = alele()->readAddress(m_ui->destination->currentText().trimmed().toStdString());
	}
	return p;
}
...
void TransactDialog::finaliseBounds(ExecutionResult const& _er)
{
	...
	quint64 baseGas = (quint64)Transaction::gasRequired(isCreation(), &m_data, m_alele->elementrem()->evmSchedule(), 0);
	...
	auto s = fromAccount();
	auto b = elementrem()->balanceAt(s, PendingBlock);
	...
	if ((qint64)m_upperBound > elementrem()->gasLimitRemaining())
	{
		// Not enough - bail.
...
void TransactDialog::determineGasRequirements()
{
	...
	m_estimationFuture = QtConcurrent::run([this, from, to]()
	{
		...
		do
		{
			...
			ExecutionResult er = elementrem()->estimateGas(from, value(), to, m_data, Invalid256, gasPrice(), PendingBlock, callback).second;
...
void TransactDialog::rejigData()
{
	...
	//findSecret(value() + elementrem()->gasLimitRemaining() * gasPrice());
	auto s = fromAccount();
	if (!s)
		return;
	...
	if (isCreation())
	{
		...
		htmlInfo = QString::fromStdString(info) + "<h4>Code</h4>" + QString::fromStdString(ele::disassemble(m_data)).toHtmlEscaped();
	}
	else
	{
	...
	htmlInfo += "<h4>Hex</h4>" + QString(ELE_HTML_DIV(ELE_HTML_MONO)) + QString::fromStdString(toHex(m_data)) + "</div>";
...
Secret TransactDialog::findSecret(u256 _totalReq) const
{
	...
	for (auto const& i: m_accounts)
	{
		auto b = elementrem()->balanceAt(i, PendingBlock);
		if (b >= _totalReq)
		{
			best = i;
			break;
	...
	return alele()->retrieveSecret(best);
}
...
{
	u256 n = elementrem()->countAt(fromAccount(), PendingBlock);
...
//	Secret s = findSecret(value() + fee());
	u256 nonce = m_ui->autoNonce->isChecked() ? elementrem()->countAt(fromAccount(), PendingBlock) : m_ui->nonce->value();
	auto a = fromAccount();
	auto b = elementrem()->balanceAt(a, PendingBlock);
	...
	Secret s = alele()->retrieveSecret(a);
	if (!s)
		return;
	...
	if (isCreation())
	{
		// If execution is a contract creation, add Natspec to
		// a local Natspec LEVELDB
		elementrem()->submitTransaction(s, value(), m_data, gas(), gasPrice(), nonce);
#if ELE_SOLIDITY
		...
		// TODO: cache like m_data.
		elementrem()->submitTransaction(s, value(), toAccount().first, m_data, gas(), gasPrice(), nonce);
	close();
}
...
//	Secret s = findSecret(value() + fee());
	Address from = fromAccount();
	auto b = elementrem()->balanceAt(from, PendingBlock);
	if (!from || b < value() + fee())
	{
	...
	try
	{
		ele::Block postState(elementrem()->postState());
		...
		t.forceSender(from);
		Debugger dw(alele(), this);
		Executive e(postState, elementrem()->blockChain(), 0);
		dw.populate(e, t);
		dw.exec();
	}
