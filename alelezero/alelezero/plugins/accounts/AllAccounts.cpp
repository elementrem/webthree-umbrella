...
{
	alele()->installWatch(ChainChangedFilter, [=](LocalisedLogEntries const&){ onAllChange(); });
	alele()->installWatch(PendingChangedFilter, [=](LocalisedLogEntries const&){ onAllChange(); });
...
void AllAccounts::save()
{
	...
	for (auto const& account: m_accounts)
	{
		...
		{
			alele()->keyManager().kill(account.first);
			deletedBeneficiary |= account.second.isDefault;
		}
		else if (account.second.isEdited)
		{
			if (account.second.isDefault)
				alele()->setAuthor(account.first);

			alele()->keyManager().changeName(account.first, account.second.name.toStdString());
		}
	}
...
void AllAccounts::refresh()
{
	...
	Addresses as;
	if (onlyKnown)
		as = alele()->allKnownAddresses();
#if ELE_FATDB
	else
		as = elementrem()->addresses();
#endif // ELE_FATDB

	std::map<Address, AccountData> newAccounts;
	for (auto const& i: as)
	{
		bool isContract = (elementrem()->codeHashAt(i) != EmptySHA3);
		...
		AccountData data;
		data.address = i;
		data.name = QString::fromStdString(alele()->toName(i));
		data.isContract = isContract;
		data.isDefault = alele()->author() == i;
...
void AllAccounts::on_accounts_currentItemChanged()
{
	...
	if (auto item = m_ui->accounts->currentItem())
	{
		...
		m_ui->balanceValue->setText(QString::fromStdString(formatBalance(elementrem()->balanceAt(address))));
		m_ui->codeValue->setText(QString::fromStdString(toHex(elementrem()->codeAt(address))));
		...
		connect(m_ui->deleteButton, &QPushButton::clicked, [this, address, item] ()
		{
			...
			if (!a.isDeleted)
			{
				QString message = QString::fromStdString("Account " + alele()->keyManager().accountName(address) + " (" + alele()->toReadable(address) + ") has " + formatBalance(alele()->elementrem()->balanceAt(address)) + " in it.\r\nIt, and any contract that this account can access, will be lost forever if you continue. Do NOT continue unless you know what you are doing.\n" "Are you sure you want to continue? \r\n If so, type 'YES' to confirm.");
			
				QString s = QInputDialog::getText(m_ui->detailsView, QString::fromStdString("Kill Account " + alele()->keyManager().accountName(address) + "?!"), message, QLineEdit::Normal, "NO");
				if (s != "YES")
					return;
			}
