...
void ImportKey::import()
{
	...
	auto updateAddress = [&](){
		...
		try
		{
			lastAddress = ele::toAddress(as);
			u.addressOut->setText(QString::fromStdString(alele()->toReadable(lastAddress)));
		}
		catch (...)
		{
	...
	if (d.exec() == QDialog::Accepted)
	{
		...
		if (lastKey.empty() && !lastSecret)
			alele()->keyManager().importExistingBrain(a, n, h);
		else if (!lastKey.empty() && !lastSecret)
		{
			h256 ph;
			DEV_IGNORE_EXCEPTIONS(ph = h256(u.passwordHash->text().toStdString()));
			alele()->keyManager().importExisting(alele()->keyManager().store().importKey(lastKey), n, a, ph, h);
		}
		else
		{
			bool mp = u.noPassword->isChecked();
			...
			if (mp)
				alele()->keyManager().import(lastSecret, n);
			else
				alele()->keyManager().import(lastSecret, n, p, h);
		}
