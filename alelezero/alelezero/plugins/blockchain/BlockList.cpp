	...
	m_ui(new Ui::BlockList)
{
	...
	m_newBlockWatch = alele()->installWatch(ChainChangedFilter, [=](LocalisedLogEntries const&){
		refreshBlocks();
	});
	m_pendingWatch = alele()->installWatch(PendingChangedFilter, [=](LocalisedLogEntries const&){
		refreshBlocks();
	});
	refreshBlocks();
}
...
BlockList::~BlockList()
{
	alele()->uninstallWatch(m_newBlockWatch);
	alele()->uninstallWatch(m_pendingWatch);
}
...
void BlockList::refreshBlocks()
{
	...
	auto const& bc = elementrem()->blockChain();
	...
	auto showBlock = [&](h256 const& h) {
		...
		if (h == PendingBlockHash)
			blockItem = new QListWidgetItem(QString("#%1 <pending>").arg(elementrem()->pendingDetails().number), m_ui->blocks);
		else
		{
			auto d = bc.details(h);
		...
		unsigned n = 0;
		try {
			for (Transaction const& t: (h == PendingBlockHash) ? elementrem()->pending() : elementrem()->transactions(h))
			{
				QString s = t.receiveAddress() ?
					QString("    %2 %5> %3: %1 [%4]")
						.arg(formatBalance(t.value()).c_str())
						.arg(QString::fromStdString(alele()->toReadable(t.safeSender())))
						.arg(QString::fromStdString(alele()->toReadable(t.receiveAddress())))
						.arg((unsigned)t.nonce())
						.arg(elementrem()->codeAt(t.receiveAddress()).size() ? '*' : '-') :
					QString("    %2 +> %3: %1 [%4]")
						.arg(formatBalance(t.value()).c_str())
						.arg(QString::fromStdString(alele()->toReadable(t.safeSender())))
						.arg(QString::fromStdString(alele()->toReadable(right160(sha3(rlpList(t.safeSender(), t.nonce()))))))
						.arg((unsigned)t.nonce());
...
void BlockList::refreshInfo()
{
	...
	if (auto item = m_ui->blocks->currentItem())
	{
		...
		if (h == PendingBlockHash)
		{
			header = elementrem()->pendingInfo();
			details = elementrem()->pendingDetails();
		}
		else
		{
			details = elementrem()->blockChain().details(h);
			blockData = elementrem()->blockChain().block(h);
			block = RLP(blockData);
		...
		if (item->data(Qt::UserRole + 1).isNull())
		{
			...
			s << "<div>Beneficiary: <b>" << htmlEscaped(alele()->toReadable(header.author())) << " " << header.author() << "</b>" << "</div>";
			s << "<div>Difficulty: <b>" << header.difficulty() << "</b>" << "</div>";
			if (h != PendingBlockHash)
			{
				for (auto const& i: elementrem()->sealEngine()->jsInfo(header))
			...
			s << "<div>State root: " << ELE_HTML_SPAN(ELE_HTML_MONO) << header.stateRoot().hex() << "</span></div>";
			s << "<div>Extra data: " << ELE_HTML_SPAN(ELE_HTML_MONO) << toHex(header.extraData()) << "</span></div>";
			if (!!header.logBloom())
			...
			if (h != PendingBlockHash)
				for (auto u: block[2])
				{
					...
					s << line << "Author: <b>" << htmlEscaped(alele()->toReadable(uncle.author())) << " " << uncle.author() << "</b>" << "</div>";
					for (auto const& i: elementrem()->sealEngine()->jsInfo(header))
			...
			if (header.parentHash())
				s << "<div>Pre: <b>" << BlockHeader(elementrem()->blockChain().block(header.parentHash())).stateRoot() << "</b>" << "</div>";
			else
			...
			{
				TransactionReceipt r = h == PendingBlockHash ? elementrem()->postState().receipt(ii) : elementrem()->blockChain().receipts(h).receipts[ii];
			...
			if (h != PendingBlockHash)
			{
				s << "<div>Dump: " ELE_HTML_SPAN(ELE_HTML_MONO) << toHex(block[0].data()) << "</span>" << "</div>";
				s << "<div>Receipts-Hex: " ELE_HTML_SPAN(ELE_HTML_MONO) << toHex(elementrem()->blockChain().receipts(h).rlp()) << "</span></div>";
			}
		}
		else
		{
			unsigned txi = item->data(Qt::UserRole + 1).toInt();
			Transaction tx = h == PendingBlockHash ? elementrem()->pending()[txi] : Transaction(block[1][txi].data(), CheckTransaction::Everything);
			auto ss = tx.safeSender();
			h256 th = sha3(rlpList(ss, tx.nonce()));
			TransactionReceipt receipt = h == PendingBlockHash ? elementrem()->postState().receipt(txi) : elementrem()->blockChain().receipts(h).receipts[txi];

			s << "<h3>" << th << "</h3>";
			s << "<h4>" << h << "[<b>" << txi << "</b>]</h4>";
			s << "<div>From: <b>" << htmlEscaped(alele()->toReadable(ss)) << " " << ss << "</b>" << "</div>";
			if (tx.isCreation())
				s << "<div>Creates: <b>" << htmlEscaped(alele()->toReadable(right160(th))) << "</b> " << right160(th) << "</div>";
			else
				s << "<div>To: <b>" << htmlEscaped(alele()->toReadable(tx.receiveAddress())) << "</b> " << tx.receiveAddress() << "</div>";
			...
			s << "<div>Msg: <b>" << tx.sha3(ele::WithoutSignature) << "</b>" << "</div>";
			if (!tx.data().empty())
			{
				if (tx.isCreation())
			...
			s << "<div>Hex: " ELE_HTML_SPAN(ELE_HTML_MONO) << toHex(block[1][txi].data()) << "</span></div>";
			s << "<hr/>";
			if (!!receipt.bloom())
			...
			s << "<div>Receipt-Hex: " ELE_HTML_SPAN(ELE_HTML_MONO) << toHex(receipt.rlp()) << "</span></div>";
			s << "<h4>Diff</h4>" << alele()->toHTML(h == PendingBlockHash ? elementrem()->diff(txi, PendingBlock) : elementrem()->diff(txi, h));
...
void BlockList::debugCurrent()
{
	...
	if (QListWidgetItem* item = m_ui->blocks->currentItem())
		if (!item->data(Qt::UserRole + 1).isNull())
		{
			...
			bytes t = h == PendingBlockHash ? elementrem()->pending()[txi].rlp() : elementrem()->blockChain().transaction(h, txi);
			State s = h == PendingBlockHash ? elementrem()->state(txi) : elementrem()->state(txi, h);
			BlockHeader bi = h == PendingBlockHash ? elementrem()->pendingInfo() : elementrem()->blockChain().info(h);
			Executive e(s, EnvInfo(bi, elementrem()->blockChain().lastHashes(bi.parentHash())), alele()->elementrem()->sealEngine());
			Debugger dw(alele(), zero());
...
void BlockList::dumpState(bool _post)
{
	...
	if (QListWidgetItem* item = m_ui->blocks->currentItem())
		if (!item->data(Qt::UserRole + 1).isNull())
		{
			...
			{
				if (item->data(Qt::UserRole + 1).isNull())
					(h == PendingBlockHash ? elementrem()->postState() : elementrem()->block(h)).state().streamJSON(f);
				else
					(h == PendingBlockHash ? elementrem()->state(txi) : elementrem()->state(txi, h)).streamJSON(f);
			}
		}
#else
	(void)_post;
