...
AleleFace* ExportStateDialog::alele() const
{
	return m_main->alele();
}
...
{
	BlockChain const& bc = alele()->elementrem()->blockChain();
...
void ExportStateDialog::fillContracts()
{
	...
	try
	{
		for (auto i: alele()->elementrem()->addresses(m_block))
		{
			string r = alele()->toReadable(i);
			(new QListWidgetItem(QString("%2: %1 [%3]").arg(formatBalance(alele()->elementrem()->balanceAt(i)).c_str()).arg(QString::fromStdString(r)).arg((unsigned)alele()->elementrem()->countAt(i)), alele()->elementrem()->codeAt(i).empty() ? m_ui->accounts : m_ui->contracts))
...
void ExportStateDialog::generateJSON()
{
	...
	for(QListWidgetItem* item: m_ui->accounts->selectedItems())
	{
		...
		json << prefix << "\t\"" << toHex(address.ref()) << "\": {  \"mey\": \"" << alele()->elementrem()->balanceAt(address, m_block) << "\" }";
		prefix = ",\n";
	}
	for(QListWidgetItem* item: m_ui->contracts->selectedItems())
	{
		auto hba = item->data(Qt::UserRole).toByteArray();
		auto address = Address((byte const*)hba.data(), Address::ConstructFromPointer);
		json << prefix << "\t\"" << toHex(address.ref()) << "\":\n\t{\n\t\t\"mey\": \"" << alele()->elementrem()->balanceAt(address, m_block) << "\",\n";
		json << "\t\t\"code\": \"" << toHex(alele()->elementrem()->codeAt(address, m_block)) << "\",\n";
		std::unordered_map<u256, u256> storage = alele()->elementrem()->storageAt(address, m_block);
		if (!storage.empty())
		{
			json << "\t\t\"storage\":\n\t\t{\n";
