...
{
	return static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->bid();
}

u256 GasPricing::ask() const
{
	return static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->ask();
}

void GasPricing::setPrices(u256 const& _bid, u256 const& _ask)
{
	static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->setBid(_bid);
	static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->setAsk(_ask);
}

void GasPricing::readSettings(QSettings const& _s)
{
	static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->setAsk(u256(_s.value("askPrice", QString::fromStdString(toString(DefaultGasPrice))).toString().toStdString()));
	static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->setBid(u256(_s.value("bidPrice", QString::fromStdString(toString(DefaultGasPrice))).toString().toStdString()));
}

void GasPricing::writeSettings(QSettings& _s)
{
	_s.setValue("askPrice", QString::fromStdString(toString(static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->ask())));
	_s.setValue("bidPrice", QString::fromStdString(toString(static_cast<TrivialGasPricer*>(alele()->elementrem()->gasPricer().get())->bid())));
}
