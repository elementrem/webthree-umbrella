...
#include <libalele/AleleWhisper.h>
...
static shh::Topics topicFromText(QString _s)
{
	...
	{
		QRegExp r("(@|\\$)?\"([^\"]*)\"(\\s.*)?");
		QRegExp d("(@|\\$)?([0-9]+)(\\s*(element)|(finney)|(szabo))?(\\s.*)?");
	...
	m_ui(new Ui::Whisper)
{
	...
	connect(zero()->whisperFace(), &AleleWhisper::onNewId, this, &Whisper::addNewId);
