...
#
# A script to build the different elementrem repositories. For usage look
...
REPOS_MSVC_SLN_MAP=("webthree-helpers/utils:utils.sln"
	"libweb3core:dev.sln"
	"libelementrem:elementrem.sln"
	"webthree:webthree.sln"
	"solidity:solidity.sln"
	"alelezero:alelezero.sln"
	"mix:mix.sln"
)

function get_repo_sln() {
	if [[ $1 == "" ]]; then
		echo "ELEUPDATE - ERROR: get_repo_sln() function called without an argument."
		exit 1
	fi
	...
	if [[ $REPO_MSVC_SLN == "" ]]; then
		echo "ELEBUILD - ERROR: Requested MSVC Solution of unknown repo: ${1}."
		exit 1
	fi
}
...
function print_help {
	echo "Usage: elebuild.sh [extra-options]"
	echo "Arguments:"
...
do
	echo -e "\nELEBUILD - INFO: Starting build of ${repository} for requested project ${REQUESTED_PROJECT}.\n";
	cd $repository >/dev/null 2>/dev/null
	if [[ $? -ne 0 ]]; then
		echo "ELEBUILD - INFO: Skipping ${repository} because directory does not exit";
	...
	if [[ $NOGIT -eq 0 && $BRANCH != $REQUESTED_BRANCH ]]; then
		echo "ELEBUILD - WARNING: ${repository} was in ${BRANCH} branch, while building on ${REQUESTED_BRANCH} was requested.";
		git checkout $REQUESTED_BRANCH
		if [[ $? -ne 0 ]]; then
			echo "ELEBUILD - WARNING: Checking out branch ${REQUESTED_BRANCH} for ${repository} failed. Will try to build with current branch ...";
		fi
	fi
	...
	if [[ $OSTYPE == "cygwin" ]]; then
		...
		source "${SCRIPT_DIR}/elewindowsenv.sh"
		cmake .. -G "Visual Studio 12 2013 Win64"
		if [[ $? -ne 0 ]]; then
			echo "ELEBUILD - ERROR: cmake configure phase for repository \"$repository\" failed.";
			exit 1
		fi
		...
		if [[ $? -ne 0 ]]; then
			echo "ELEBUILD - ERROR: Building repository \"$repository\" failed.";
			exit 1
		fi
	else
