...
#
# A script to update the different elementrem repositories to latest develop
...
source "${SCRIPT_DIR}/elebuildcommon.sh"
...
REPO_URL=""
BUILD_PR="none"
REPOS_MAP=("webthree-helpers:https://github.com/elementrem/webthree-helpers"
	   "tests:https://github.com/elementrem/tests"
	   "libweb3core:https://github.com/elementrem/libweb3core"
	   "libelementrem:https://github.com/elementrem/libelementrem"
	   "libwhisper:https://github.com/elementrem/libwhisper"
	   "webthree:https://github.com/elementrem/webthree"
	   "web3.js:https://github.com/elementrem/web3.js"
	   "solidity:https://github.com/elementrem/solidity"
	   "alelezero:https://github.com/elementrem/alelezero"
	   "mix:https://github.com/elementrem/mix")
REPOS_SSH_MAP=("webthree-helpers:git@github.com:elementrem/webthree-helpers.git"
	   "tests:git@github.com:elementrem/tests.git"
	   "libweb3core:git@github.com:elementrem/libweb3core.git"
	   "libelementrem:git@github.com:elementrem/libelementrem.git"
	   "libwhisper:git@github.com:elementrem/libwhisper.git"
	   "webthree:git@github.com:elementrem/webthree.git"
	   "web3.js:git@github.com:elementrem/web3.js.git"
	   "solidity:git@github.com:elementrem/solidity.git"
	   "alelezero:git@github.com:elementrem/alelezero.git"
	   "mix:git@github.com:elementrem/mix.git")

function get_repo_url() {
	if [[ $1 == "" ]]; then
		echo "ELEUPDATE - ERROR: get_repo_url() function called without an argument."
		exit 1
	fi
	...
	if [[ $REPO_URL == "" ]]; then
		echo "ELEUPDATE - ERROR: Requested url of unknown repo: ${1}."
		exit 1
	fi
}
...
function get_repo_branch() {
	if [[ $1 == "" ]]; then
		echo "ELEUPDATE - ERROR: get_repo_branch() function called without an argument."
		exit 1
	fi
	...
	case $1 in
		...
		"libweb3core")
			...
			;;
		"libelementrem")
			set_requested_branch $LIBELEMENTREM_BRANCH
			;;
		"webthree")
		...
		"solidity")
			...
			;;
		"alelezero")
			set_requested_branch $ALELEZERO_BRANCH
			;;
		"mix")
		...
		"tests")
			...
			;;
		*)
			echo "ELEUPDATE - ERROR: Unrecognized repo argument at get_repo_branch() \"$1\".";
			exit 1
			;;
	esac
	echo "ELEUPDATE - INFO: get_repo_branch() setting branch for $1 to ${REQUESTED_BRANCH}."
	#if not develop or master we got some work to do
	if [[ $REQUESTED_BRANCH != "develop" && $REQUESTED_BRANCH != "master" ]]; then
		if [[ $GH_PR_USER == "" ]];then
			GH_PR_USER="elementrem"
			echo "ELEUPDATE - INFO: get_repo_branch() no GH_PR_USER given, defaulting to elementrem repo."
		fi
		#fetch the requested branch
		git fetch https://github.com/${GH_PR_USER}/$1 ${REQUESTED_BRANCH}
		if [[ $? -ne 0 ]]; then
			if [[ $GH_PR_USER != "elementrem" ]]; then
				echo "ELEUPDATE - WARNING: Could not find ${REQUESTED_BRANCH} of ${1} from the fork of ${GH_PR_USER}. Trying the elementrem upstream"
				git fetch https://github.com/elementrem/$1 ${REQUESTED_BRANCH}
				if [[ $? -eq 0 ]]; then
					git checkout FETCH_HEAD
					echo "ELEUPDATE - INFO: Found ${REQUESTED_BRANCH} of ${1} in the elementrem upstream"
					return
				fi
			fi
			echo "ELEUPDATE - ERROR: Could not fetch ${REQUESTED_BRANCH} of ${1} for ${GH_PR_USER} or from the elementrem upstream.. Defaulting to develop."
			REQUESTED_BRANCH="develop"
                else
			git checkout FETCH_HEAD
		fi
...
function print_help {
	echo "Usage: eleupdate.sh [options]"
	echo "Arguments:"
	...
	if [[ ${REQUESTED_ARG} != "" ]]; then
		case $REQUESTED_ARG in
			...
			"branch")
				...
				;;
			"project")
				set_repositories "ELEUPDATE" $arg
				;;
			"build-pr")
				BUILD_PR=$arg
				;;
			*)
				echo "ELEUPDATE - ERROR: Unrecognized argument \"$arg\".";
				print_help
				exit 1
		esac
		REQUESTED_ARG=""
...
if [[ ${REQUESTED_ARG} != "" ]]; then
	echo "ELEUPDATE - ERROR: Expected value for the \"${REQUESTED_ARG}\" argument";
	exit 1
fi

if [[ $USE_ALL -eq 1 && ($REQUESTED_PROJECT != "alelezero" || $REQUESTED_PROJECT != "mix") ]]; then
...
do
	...
	echo "ELEUPDATE - INFO: Starting update process of ${repository} for requested project ${REQUESTED_PROJECT}";
	CLONED_THE_REPO=0
	cd $repository >/dev/null 2>/dev/null
	if [[ $? -ne 0 ]]; then
		if [[ $REQUESTED_PROJECT == "" ]]; then
			echo "ELEUPDATE - INFO: Skipping ${repository} because directory does not exit";
			cd $ROOT_DIR
			continue
		else
			echo "ELEUPDATE - INFO: Repository ${repository} for requested project ${REQUESTED_PROJECT} did not exist. Cloning ..."
	...
	if [[ $BUILD_PR == "none" ]]; then
		...
		if [[ $? -ne 0 ]]; then
			echo "ELEUPDATE - ERROR: Could not checkout \"${REQUESTED_BRANCH}\" for ${repository}."
			exit 1
		else
			echo "ELEUPDATE - INFO: Checked out branch ${REQUESTED_BRANCH} for repository ${repository}."
		fi
	else
		get_repo_branch $repository
	fi
	...
	if [[ $CHECKOUT_HEX -eq 1 ]]; then
		echo "ELEUPDATE - INFO: Checking out commit ${BUILD_PR} for ${repository} as requested."
		get_repo_url $repository
		...
		echo "ELEUPDATE - INFO: Performing simple pull for ${repository} ..."
		git pull $SHALLOW_FETCH
		if [[ $? -ne 0 ]]; then
			echo "ELEUPDATE - ERROR: Doing a simple pull for ${repository} failed. Skipping this repository ..."
		fi
		# Init new and update submodules
		git submodule update --init --force
		cd $ROOT_DIR
	...
	if [[ $REQUESTED_BRANCH != "develop" && $REQUESTED_BRANCH != "master" ]];then
		...
		git checkout -f $REQUESTED_BRANCH
		if [[ $? -ne 0 ]]; then
			echo "ELEUPDATE - ERROR: Could not check out branch ${REQUESTED_BRANCH} for repository ${repository}. Skipping ..."
		else
			echo "ELEUPDATE - INFO: Checked out branch ${REQUESTED_BRANCH} for repository ${repository}."
		fi
		# Init new and update submodules
	...
	if [[ $CLONED_THE_REPO -eq 0 ]]; then
		if [[ $REQUESTED_BRANCH == "develop" || $REQUESTED_BRANCH == "master" ]]; then
			...
			echo "ELEUPDATE - INFO: Make sure we are in $REQUESTED_BRANCH"
			git checkout -f $REQUESTED_BRANCH
		fi
	...
	if [[ $? -ne 0 ]]; then
		if [[ $DO_SIMPLE_PULL -eq 1 ]]; then
			echo "ELEUPDATE - INFO: ${repository} failed to pull ${REQUESTED_BRANCH}. Performing a simple pull anyway ..."
			git pull $SHALLOW_FETCH
			if [[ $? -ne 0 ]]; then
				echo "ELEUPDATE - ERROR: Doing a simple pull for ${repository} failed. Skipping this repository ..."
			fi
			...
			echo "ELEUPDATE - ERROR: Pulling changes for repository ${repository} from ${UPSTREAM} into the ${REQUESTED_BRANCH} branch failed."
		fi
		cd $ROOT_DIR
		continue
	fi
	...
	if [[ $NO_PUSH -eq 0 && $UPSTREAM != $ORIGIN ]]; then
		...
		if [[ $? -ne 0 ]]; then
			echo "ELEUPDATE - ERROR: Could not update origin ${ORIGIN} of repository ${repository} for the ${REQUESTED_BRANCH}."
			cd $ROOT_DIR
			continue
		fi
	fi
	echo "ELEUPDATE - INFO: ${repository} succesfully updated!"
	cd $ROOT_DIR
done
