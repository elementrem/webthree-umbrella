...
# git clone --recursive -b develop https://github.com/elementrem/webthree-umbrella
...
for component in webthree-helpers libweb3core libelementrem solidity
do
  cd "$WORKSPACE"
  if [ -x $component ]
  then
    ...
    git clone https://github.com/elementrem/$component
  fi
...
for component in webthree-helpers/utils libweb3core libelementrem solidity
do
...
emcmake cmake \
  ...
  -DEle_EVMCORE_LIBRARY="$WORKSPACE"/libelementrem/build/libevmcore/libevmcore.a \
  -DEle_EVMASM_LIBRARY="$WORKSPACE"/solidity/build/libevmasm/libevmasm.a \
  -DUtils_SCRYPT_LIBRARY="$WORKSPACE"/webthree-helpers/utils/build/libscrypt/libscrypt.a \
  -DELHASHCL=0 -DEVMJIT=0 -DELE_STATIC=1 -DSOLIDITY=1 -DGUI=0 -DFATDB=0 -DTESTS=0 -DTOOLS=0 \
  ..
emmake make -j 4
done
