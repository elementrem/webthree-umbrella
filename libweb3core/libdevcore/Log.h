...
#define ELE_THREAD_CONTEXT(name) for (std::pair<dev::ThreadContext, bool> __ele_thread_context(name, true); p.second; p.second = false)
...
public:
	...
	void comment(std::string const& _t)
	{
		switch (m_logTag)
		{
		case LogTag::Url: m_sstr << EleNavyUnder; break;
		case LogTag::Error: m_sstr << EleRedBold; break;
		case LogTag::Special: m_sstr << EleWhiteBold; break;
		default:;
		}
		m_sstr << _t << EleReset;
	...
	void append(unsigned long _t) { m_sstr << EleBlue << _t << EleReset; }
	void append(long _t) { m_sstr << EleBlue << _t << EleReset; }
	void append(unsigned int _t) { m_sstr << EleBlue << _t << EleReset; }
	void append(int _t) { m_sstr << EleBlue << _t << EleReset; }
	void append(bigint const& _t) { m_sstr << EleNavy << _t << EleReset; }
	void append(u256 const& _t) { m_sstr << EleNavy << _t << EleReset; }
	void append(u160 const& _t) { m_sstr << EleNavy << _t << EleReset; }
	void append(double _t) { m_sstr << EleBlue << _t << EleReset; }
	template <unsigned N> void append(FixedHash<N> const& _t) { m_sstr << EleTeal "#" << _t.abridged() << EleReset; }
	void append(h160 const& _t) { m_sstr << EleRed "@" << _t.abridged() << EleReset; }
	void append(h256 const& _t) { m_sstr << EleCyan "#" << _t.abridged() << EleReset; }
	void append(h512 const& _t) { m_sstr << EleTeal "##" << _t.abridged() << EleReset; }
	void append(std::string const& _t) { m_sstr << EleGreen "\"" + _t + "\"" EleReset; }
	void append(bytes const& _t) { m_sstr << EleYellow "%" << toHex(_t) << EleReset; }
	void append(bytesConstRef _t) { m_sstr << EleYellow "%" << toHex(_t) << EleReset; }
	...
	{
		m_sstr << EleWhite "[" EleReset;
		int n = 0;
		for (auto const& i: _t)
		{
			m_sstr << (n++ ? EleWhite ", " EleReset : "");
			append(i);
		}
		m_sstr << EleWhite "]" EleReset;
	}
	...
	{
		m_sstr << EleYellow "{" EleReset;
		int n = 0;
		for (auto const& i: _t)
		{
			m_sstr << (n++ ? EleYellow ", " EleReset : "");
			append(i);
		}
		m_sstr << EleYellow "}" EleReset;
	}
	...
	{
		m_sstr << EleLime "{" EleReset;
		int n = 0;
		for (auto const& i: _t)
		{
			m_sstr << (n++ ? EleLime ", " EleReset : "");
			append(i.first);
			m_sstr << (n++ ? EleLime ": " EleReset : "");
			append(i.second);
		}
...
#define DEV_STATEMENT_IF(COND) for (bool i_ele_if_ = (COND); i_ele_if_; i_ele_if_ = false)
