 ...
 * ELE_(m) d = 1;
 * ...
 * ELE_(m) { for (auto d = 10; d > 0; --d) foo(d); d = 0; }
 * @endcode
 *
...
#define DEV_GUARDED(MUTEX) \
	for (GenericGuardBool<Guard, Mutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
#define DEV_READ_GUARDED(MUTEX) \
	for (GenericGuardBool<ReadGuard, SharedMutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
#define DEV_WRITE_GUARDED(MUTEX) \
	for (GenericGuardBool<WriteGuard, SharedMutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
#define DEV_RECURSIVE_GUARDED(MUTEX) \
	for (GenericGuardBool<RecursiveGuard, RecursiveMutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
#define DEV_UNGUARDED(MUTEX) \
	for (GenericUnguardBool<Mutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
#define DEV_READ_UNGUARDED(MUTEX) \
	for (GenericUnguardSharedBool<SharedMutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
#define DEV_WRITE_UNGUARDED(MUTEX) \
	for (GenericUnguardBool<SharedMutex> __ele_l(MUTEX); __ele_l.b; __ele_l.b = false)
