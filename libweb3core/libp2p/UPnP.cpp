...
int UPnP::addRedirect(char const* _addr, int _port)
{
	...
	if (!UPNP_AddPortMapping(m_urls->controlURL, m_data->first.servicetype, port_str, port_str, _addr, "elementrem", "TCP", NULL, NULL))
		return _port;
	...
	for (unsigned i = 0; i < 10; ++i)
	{
		...
		if (!UPNP_AddPortMapping(m_urls->controlURL, m_data->first.servicetype, ext_port_str, port_str, _addr, "elementrem", "TCP", NULL, NULL))
			return _port;
	}

	// Failed. Try asking the router to give us a free external port.
	if (UPNP_AddPortMapping(m_urls->controlURL, m_data->first.servicetype, port_str, NULL, _addr, "elementrem", "TCP", NULL, NULL))
		// Failed. Exit.
		return 0;
