...
bytesConstRef JitVM::execImpl(u256& io_gas, ExtVMFace& _ext, OnOpFunc const& _onOp)
{
	...
	m_data.address      = ele2jit(fromAddress(_ext.myAddress));
	m_data.caller       = ele2jit(fromAddress(_ext.caller));
	m_data.origin       = ele2jit(fromAddress(_ext.origin));
	m_data.transferredValue = ele2jit(_ext.value);
	m_data.apparentValue = ele2jit(_ext.value);
	m_data.coinBase     = ele2jit(fromAddress(_ext.envInfo().author()));
	m_data.difficulty   = ele2jit(_ext.envInfo().difficulty());
	m_data.gasLimit     = ele2jit(_ext.envInfo().gasLimit());
	...
	m_data.codeHash		= ele2jit(_ext.codeHash);
	...
	case evmjit::ReturnCode::Suicide:
		_ext.suicide(right160(jit2ele(m_data.address)));
		break;
