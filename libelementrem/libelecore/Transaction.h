...
public:
	...
	/// @returns the amount of ELE to be transferred by this (message-call) transaction, in Mey. Synonym for endowment().
	u256 value() const { return m_value; }
	/// @returns the amount of ELE to be endowed by this (contract-creation) transaction, in Mey. Synonym for value().
	u256 endowment() const { return m_value; }

	/// @returns the base fee and thus the implied exchange rate of ELE to GAS.
...
protected:
	...
	u256 m_value;						///< The amount of ELE to be transferred by this transaction. Called 'endowment' for contract-creation transactions.
	Address m_receiveAddress;			///< The receiving address of the transaction.
	u256 m_gasPrice;					///< The base fee and thus the implied exchange rate of ELE to GAS.
