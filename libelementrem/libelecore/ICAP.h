 ...
 * 
 *
 * Elementrem-specific data structures & algorithms.
 */
...
public:
	...
	/// @returns target name. Only valid when type() == Indirect and asset() == "ELE".
	std::string const& target() const { return m_type == Indirect && m_asset == "ELE" ? m_client : EmptyString; }
