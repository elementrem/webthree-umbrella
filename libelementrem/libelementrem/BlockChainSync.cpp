...
std::ostream& dev::ele::operator<<(std::ostream& _out, SyncStatus const& _sync)
{
	_out << "protocol: " << _sync.protocolVersion << endl;
	_out << "state: " << ElementremHost::stateName(_sync.state) << " ";
...
BlockChainSync::BlockChainSync(ElementremHost& _host):
	m_host(_host),
...
{
	resetSync();
	host().foreachPeer([&](std::shared_ptr<ElementremPeer> _p)
	{
		_p->abortSync();
		return true;
	});
...
void BlockChainSync::syncPeer(std::shared_ptr<ElementremPeer> _peer, bool _force)
{
...
{
	host().foreachPeer([this](std::shared_ptr<ElementremPeer> _p)
	{
		syncPeer(_p, false);
		return true;
	});
...
void BlockChainSync::requestBlocks(std::shared_ptr<ElementremPeer> _peer)
{
...
void BlockChainSync::clearPeerDownload(std::shared_ptr<ElementremPeer> _peer)
{
...
void BlockChainSync::onPeerNewHashes(std::shared_ptr<ElementremPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes)
{
	RecursiveGuard l(x_sync);
	DEV_INVARIANT_CHECK;
