...
/**
 * @brief The ElementremPeer class
 * @todo Document fully.
 * @todo make state transitions thread-safe.
 */
class ElementremPeer: public p2p::Capability
{
	friend class ElementremHost; //TODO: remove this
	friend class BlockChainSync; //TODO: remove this

public:
	/// Basic constructor.
	ElementremPeer(std::shared_ptr<p2p::Session> _s, p2p::HostCapabilityFace* _h, unsigned _i, p2p::CapDesc const& _cap, uint16_t _capID);

	/// Basic destructor.
	virtual ~ElementremPeer();

	/// What is our name?
	static std::string name() { return "ele"; }
	...
	/// What is the elementrem subprotocol host object.
	ElementremHost* host() const;
