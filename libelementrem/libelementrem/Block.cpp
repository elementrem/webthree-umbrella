...
const char* BlockSafeExceptions::name() { return EleViolet "⚙" EleBlue " ℹ"; }
const char* BlockDetail::name() { return EleViolet "⚙" EleWhite " ◌"; }
const char* BlockTrace::name() { return EleViolet "⚙" EleGray " ◎"; }
const char* BlockChat::name() { return EleViolet "⚙" EleWhite " ◌"; }
...
bool Block::sync(BlockChain const& _bc, h256 const& _block, BlockHeader const& _bi)
{
	...
	else
	{
		...
		if (m_state.db().lookup(bi.stateRoot()).empty())	// TODO: API in State for this?
		{
			...
			cwarn << "Try rescuing the database by running: ele --rescue";
...
std::ostream& dev::ele::operator<<(std::ostream& _out, Block const& _s)
{
	(void)_s;
	return _out;
}
