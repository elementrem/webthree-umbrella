...
std::ostream& dev::ele::operator<<(std::ostream& _out, ActivityReport const& _r)
{
...
#if defined(_WIN32)
const char* ClientNote::name() { return EleTeal "^" EleBlue " i"; }
const char* ClientChat::name() { return EleTeal "^" EleWhite " o"; }
const char* ClientTrace::name() { return EleTeal "^" EleGray " O"; }
const char* ClientDetail::name() { return EleTeal "^" EleCoal " 0"; }
#else
const char* ClientNote::name() { return EleTeal "⧫" EleBlue " ℹ"; }
const char* ClientChat::name() { return EleTeal "⧫" EleWhite " ◌"; }
const char* ClientTrace::name() { return EleTeal "⧫" EleGray " ◎"; }
const char* ClientDetail::name() { return EleTeal "⧫" EleCoal " ●"; }
#endif
...
void Client::init(p2p::Host* _extNet, std::string const& _dbPath, WithExisting _forceAction, u256 _networkId)
{
	...
	auto host = _extNet->registerCapability(make_shared<ElementremHost>(bc(), m_stateDB, m_tq, m_bq, _networkId));
	m_host = host;
	_extNet->addCapability(host, ElementremHost::staticName(), ElementremHost::c_oldProtocolVersion); //TODO: remove this once v61+ protocol is common
...
ele::State Client::state(unsigned _txi) const
{
	DEV_READ_GUARDED(x_postSeal)
