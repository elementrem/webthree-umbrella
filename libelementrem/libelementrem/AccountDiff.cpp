...
char const* dev::ele::lead(AccountChange _c)
{
	switch (_c)
	{
...
std::ostream& operator<<(std::ostream& _out, dev::ele::AccountDiff const& _s)
{
	if (!_s.exist.to())
		return _out;
...
std::ostream& operator<<(std::ostream& _out, dev::ele::StateDiff const& _s)
{
	_out << _s.accounts.size() << " accounts changed:" << endl;
	dev::ele::AccountDiff d;
	_out << d;
