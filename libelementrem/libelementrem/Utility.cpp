...
bytes dev::ele::parseData(string const& _args)
{
	bytes m_data;

	boost::smatch what;
	static const boost::regex r("(!|#|@|\\$)?\"([^\"]*)\"(\\s.*)?");
	static const boost::regex d("(@|\\$)?([0-9]+)(\\s*(element)|(finney)|(szabo))?(\\s.*)?");
...
void dev::ele::upgradeDatabase(std::string const& _basePath, h256 const& _genesisHash)
{
