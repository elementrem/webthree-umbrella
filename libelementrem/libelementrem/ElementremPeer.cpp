...
ElementremPeer::ElementremPeer(std::shared_ptr<Session> _s, HostCapabilityFace* _h, unsigned _i, CapDesc const& _cap, uint16_t _capID):
...
bool ElementremPeer::isRude() const
{
	auto s = session();
	if (s)
...
unsigned ElementremPeer::askOverride() const
{
	std::string static const badGele = "Gele/v0.9.27";
	auto s = session();
	if (!s)
		return c_maxBlocksAsk;
	if (s->info().clientVersion.substr(0, badGele.size()) == badGele)
		return 1;
	bytes const& d = repMan().data(*s, name());
	return d.empty() ? c_maxBlocksAsk : RLP(d).toInt<unsigned>(RLP::LaissezFaire);
}

void ElementremPeer::setRude()
{
	auto s = session();
	if (!s)
		return;
...
void ElementremPeer::abortSync()
{
	host()->onPeerAborting();
}

ElementremHost* ElementremPeer::host() const
{
	return static_cast<ElementremHost*>(Capability::hostCapability());
}
...
void ElementremPeer::setIdle()
{
	setAsking(Asking::Nothing);
}

void ElementremPeer::requestStatus()
{
	...
	prep(s, StatusPacket, 5)
					<< (latest ? host()->protocolVersion() : ElementremHost::c_oldProtocolVersion)
					<< host()->networkId()
...
void ElementremPeer::requestBlockHeaders(unsigned _startNumber, unsigned _count, unsigned _skip, bool _reverse)
{
	if (m_asking != Asking::Nothing)
	{
...
void ElementremPeer::requestBlockHeaders(h256 const& _startHash, unsigned _count, unsigned _skip, bool _reverse)
{
	if (m_asking != Asking::Nothing)
	{
		clog(NetWarn) << "Asking headers while requesting " << ::toString(m_asking);
...
void ElementremPeer::requestBlockBodies(h256s const& _blocks)
{
	if (m_asking != Asking::Nothing)
	{
		clog(NetWarn) << "Asking headers while requesting " << ::toString(m_asking);
...
void ElementremPeer::setAsking(Asking _a)
{
	m_asking = _a;
...
bool ElementremPeer::isConversing() const
{
	return m_asking != Asking::Nothing;
}

bool ElementremPeer::isCriticalSyncing() const
{
	return m_asking == Asking::BlockHeaders || m_asking == Asking::State || (m_asking == Asking::BlockBodies && m_protocolVersion == 62);
}

bool ElementremPeer::interpret(unsigned _id, RLP const& _r)
{
	m_lastAsk = std::chrono::system_clock::to_time_t(chrono::system_clock::now());
	try
	{
	...
	case StatusPacket:
	{
		...
		setIdle();
		host()->onPeerStatus(dynamic_pointer_cast<ElementremPeer>(dynamic_pointer_cast<ElementremPeer>(shared_from_this())));
		break;
	}
	case TransactionsPacket:
	{
		host()->onPeerTransactions(dynamic_pointer_cast<ElementremPeer>(dynamic_pointer_cast<ElementremPeer>(shared_from_this())), _r);
		break;
	}
	...
	case BlockHeadersPacket:
	{
		...
		{
			setIdle();
			host()->onPeerBlockHeaders(dynamic_pointer_cast<ElementremPeer>(shared_from_this()), _r);
		}
		break;
	}
	...
	case BlockBodiesPacket:
	{
		...
		{
			setIdle();
			host()->onPeerBlockBodies(dynamic_pointer_cast<ElementremPeer>(shared_from_this()), _r);
		}
		break;
	}
	case NewBlockPacket:
	{
		host()->onPeerNewBlock(dynamic_pointer_cast<ElementremPeer>(shared_from_this()), _r);
		break;
	}
	case NewBlockHashesPacket:
	{
		...
		host()->onPeerNewHashes(dynamic_pointer_cast<ElementremPeer>(shared_from_this()), hashes);
		break;
	}
