...
/**
 * @brief Main API hub for interfacing with Elementrem.
 */
...
public:
	...
	/// Makes the given call. Nothing is recorded into the state. This cheats by creating a null address and endowing it with a lot of ELE.
	ExecutionResult call(Address _dest, bytes const& _data = bytes(), u256 _gas = 125000, u256 _value = 0, u256 _gasPrice = 1 * element, Address const& _from = Address());
	...
	/// Get the block.
	dev::ele::Block block(h256 const& _blockHash, PopulationStatistics* o_stats = nullptr) const;
	/// Get the state of the given block part way through execution, immediately before transaction
	/// index @a _txi.
	dev::ele::State state(unsigned _txi, h256 const& _block) const;
	/// Get the state of the currently pending block part way through execution, immediately before
	/// transaction index @a _txi.
	dev::ele::State state(unsigned _txi) const;

	/// Get the object representing the current state of Elementrem.
	dev::ele::Block postState() const { ReadGuard l(x_postSeal); return m_postSeal; }
...
protected:
	...
	std::weak_ptr<ElementremHost> m_host;		///< Our Elementrem Host. Don't do anything if we can't lock.
