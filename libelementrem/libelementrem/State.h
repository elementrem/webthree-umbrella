...
//#define ELE_ALLOW_EMPTY_BLOCK_AND_STATE 1

class SealEngineFace;

/**
 * @brief Model of an Elementrem state, essentially a facade for the trie.
...
public:
...
#if ELE_ALLOW_EMPTY_BLOCK_AND_STATE
	...
	/// Populate the state from the given AccountMap. Just uses dev::ele::commit().
	void populateFrom(AccountMap const& _map);

	/// @returns the set containing all addresses currently in use in Elementrem.
	/// @warning This is slowslowslow. Don't use it unless you want to lock the object for seconds or minutes at a time.
	/// @throws InterfaceNotSupported if compiled without ELE_FATDB.
