...
#include <libelecore/Precompiled.h>
using namespace std;
using namespace dev;
...
AccountMap dev::ele::jsonToAccountMap(std::string const& _json, u256 const& _defaultNonce, AccountMaskMap* o_mask, PrecompiledContractMap* o_precompiled)
{
	...
	for (auto const& account: o.count("alloc") ? o["alloc"].get_obj() : o.count("accounts") ? o["accounts"].get_obj() : o)
	{
		...
		bool haveBalance = (o.count("mey") || o.count("finney") || o.count("balance"));
		...
		if (haveStorage || haveCode || haveNonce || haveBalance)
		{
			u256 balance = 0;
			if (o.count("mey"))
				balance = u256Safe(o["mey"].get_str());
