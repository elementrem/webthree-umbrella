...
class ElementremHost;
class BlockQueue;
class ElementremPeer;
...
{
public:
	BlockChainSync(ElementremHost& _host);
	~BlockChainSync();
	...
	void onPeerBlockHeaders(std::shared_ptr<ElementremPeer> _peer, RLP const& _r);
...
private:
	...
	ElementremHost& host() { return m_host; }
	ElementremHost const& host() const { return m_host; }

	void resetSync();
	void syncPeer(std::shared_ptr<ElementremPeer> _peer, bool _force);
	void requestBlocks(std::shared_ptr<ElementremPeer> _peer);
	void clearPeerDownload(std::shared_ptr<ElementremPeer> _peer);
...
private:
	...
	std::map<std::weak_ptr<ElementremPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<ElementremPeer>>> m_headerSyncPeers; ///< Peers to m_downloadingSubchain number map
	std::map<std::weak_ptr<ElementremPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<ElementremPeer>>> m_bodySyncPeers; ///< Peers to m_downloadingSubchain number map
