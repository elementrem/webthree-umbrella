...
#define ELE_CHECK_EQUAL(x, y) { dev::WriteGuard(x_boostTest); BOOST_CHECK_EQUAL(x, y); }
#define ELE_CHECK_EQUAL_COLLECTIONS(xb, xe, yb, ye) { dev::WriteGuard(x_boostTest); BOOST_CHECK_EQUAL_COLLECTIONS(xb, xe, yb, ye); }
#define ELE_REQUIRE(x) { dev::WriteGuard(x_boostTest); BOOST_REQUIRE(x); }
...
{
	void enumerateBlockchains(std::function<void(Json::Value const&, dev::ele::BlockChain const&, dev::ele::State state)> callback) const;
};

struct ClientBaseFixture: public BlockChainFixture
{
	void enumerateClients(std::function<void(Json::Value const&, dev::ele::ClientBase&)> callback) const;
};
