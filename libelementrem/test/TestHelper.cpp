...
void connectClients(Client& c1, Client& c2)
{
	(void)c1;
	(void)c2;
	// TODO: Move to WebThree. ele::Client no longer handles networking.
#if 0
	short c1Port = 20000;
	short c2Port = 21000;
...
//	cdebug << "Block mined" << Elhash::boundary(_bi).hex() << Elhash::nonce(_bi) << _bi.hash(WithoutSeal).hex();
...
ImportTest::ImportTest(json_spirit::mObject& _o, bool isFiller, testType testTemplate):
	m_statePre(0, OverlayDB(), ele::BaseState::Empty),
	m_statePost(0, OverlayDB(), ele::BaseState::Empty),
	m_testObject(_o)
{
...
bytes ImportTest::executeTest(ele::Network _sealEngineNetwork)
{
	ExecutionResult res;
	ele::State tmpState = m_statePre;
	try
	{
...
void ImportTest::importTransaction (json_spirit::mObject const& _o, ele::Transaction& o_tr)
{
...
int ImportTest::exportTest(bytes const& _output)
{
	...
	{
		ele::AccountMaskMap stateMap;
		State expectState(0, OverlayDB(), ele::BaseState::Empty);
...
void userDefinedTest(std::function<void(json_spirit::mValue&, bool)> doTests)
{
	...
	{
		cnote << "Missing user test specification\nUsage: testele --singletest <filename> <testname>\n";
		return;
	}
...
void executeTests(const string& _name, const string& _testPathAppendix, const boost::filesystem::path _pathToFiller, std::function<void(json_spirit::mValue&, bool)> doTests)
{
	...
	try
	{
		...
		BOOST_REQUIRE_MESSAGE(s.length() > 0, "Contents of " + testPath + "/" + _name + ".json is empty. Have you cloned the 'tests' repo branch develop and set ELEMENTREM_TEST_PATH to its path?");
		json_spirit::read_string(s, v);
	...
	for (auto i = 0; i < argc; ++i)
	{
		...
		else if (arg == "--sealengine")
		{
			if (std::string{argv[i + 1]} == "Frontier")
				sealEngineNetwork = ele::Network::FrontierTest;
			else if (std::string{argv[i + 1]} == "Homestead")
				sealEngineNetwork = ele::Network::HomesteadTest;
			else
				sealEngineNetwork = ele::Network::Test;
			++i;
		}
...
void updateElhashSeal(dev::ele::BlockHeader& _header, h256 const& _mixHash, h64 const& _nonce)
{
	Elhash::setNonce(_header, _nonce);
	Elhash::setMixHash(_header, _mixHash);
}
