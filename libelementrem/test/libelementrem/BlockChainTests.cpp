...
ele::Network TestBlockChain::s_sealEngineNetwork = ele::Network::FrontierTest;
...
struct ChainBranch
{
	...
	static void forceBlockchain(string const& chainname)
	{
		...
		if (chainname == "Frontier")
			dev::test::TestBlockChain::s_sealEngineNetwork = ele::Network::FrontierTest;
		if (chainname == "Homestead")
			dev::test::TestBlockChain::s_sealEngineNetwork = ele::Network::HomesteadTest;
		if (chainname == "TestFtoH5")
			dev::test::TestBlockChain::s_sealEngineNetwork = ele::Network::Test;
	}
...
private:
	static ele::Network s_tempBlockchainNetwork;
};

ele::Network ChainBranch::s_tempBlockchainNetwork = ele::Network::Test;
...
void overwriteBlockHeaderForTest(mObject const& _blObj, TestBlock& _block, ChainBranch const& _chainBranch)
{
	...
	if (ho.size() != 14)
	{
		...
		if (ho.count("RelTimestamp"))
		{
			...
			tmp.setDifficulty(((const Elhash*)sealEngine)->calculateDifficulty(tmp, parentHeader));
			this_thread::sleep_for(chrono::seconds((int)toInt(ho["RelTimestamp"])));
		}

		Elhash::setMixHash(tmp, ho.count("mixHash") ? h256(ho["mixHash"].get_str()) : Elhash::mixHash(header));
		Elhash::setNonce(tmp, ho.count("nonce") ? Nonce(ho["nonce"].get_str()) : Elhash::nonce(header));
		tmp.noteDirty();
	}
	else
	{
...
void overwriteUncleHeaderForTest(mObject& uncleHeaderObj, TestBlock& uncle, std::vector<TestBlock> const& uncles, ChainBranch const& _chainBranch)
{
	...
	if (uncleHeaderObj.count("populateFromBlock"))
	{
		...
		if (number < importedBlocks.size())
		{
			...
			if (uncleHeaderObj.count("RelTimestamp"))
			{
				...
				uncleHeader.setDifficulty(((const Elhash*)sealEngine)->calculateDifficulty(uncleHeader, parentHeader));
	...
	if (overwrite != "false")
	{
		uncleHeader = constructHeader(
			...
			overwrite == "difficulty" ? toInt(uncleHeaderObj.at("difficulty"))
									  :	overwrite == "timestamp" ? ((const Elhash*)sealEngine)->calculateDifficulty(uncleHeader, importedBlocks.at((size_t)uncleHeader.number() - 1).blockHeader())
	...
	{
		if (overwrite == "nonce")
			Elhash::setNonce(uncleHeader, Nonce(uncleHeaderObj["nonce"].get_str()));
		if (overwrite == "mixHash")
			Elhash::setMixHash(uncleHeader, h256(uncleHeaderObj["mixHash"].get_str()));
...
mObject writeBlockHeaderToJson(BlockHeader const& _bi)
{
	...
	o["mixHash"] = toString(Elhash::mixHash(_bi));
	o["nonce"] = toString(Elhash::nonce(_bi));
...
void checkBlocks(TestBlock const& _blockFromFields, TestBlock const& _blockFromRlp, string const& _testname)
{
	...
	//BOOST_CHECK_MESSAGE(Elhash::mixHash(blockHeaderFromFields) == Elhash::mixHash(blockFromRlp), _testname + "mixHash in given RLP not matching the block mixHash!");
	//BOOST_CHECK_MESSAGE(Elhash::nonce(blockHeaderFromFields) == Elhash::nonce(blockFromRlp), _testname + "nonce in given RLP not matching the block nonce!");
