...
#include <libelementrem/GenesisInfo.h>
...
{
	BOOST_WARN(string(BlockChainDebug::name()) == string(EleBlue "☍" EleWhite " ◇"));
	BOOST_WARN(string(BlockChainWarn::name()) == string(EleBlue "☍" EleOnRed EleBlackBold " ✘"));
	BOOST_WARN(string(BlockChainNote::name()) == string(EleBlue "☍" EleBlue " ℹ"));
	BOOST_WARN(string(BlockChainChat::name()) == string(EleBlue "☍" EleWhite " ◌"));
...
BOOST_AUTO_TEST_CASE(opendb)
{
	...
	block.sync(bc);
	dev::ele::mine(block, bc, bc.sealEngine());
...
BOOST_AUTO_TEST_CASE(rebuild)
{
	...
	TestBlock genesisCopy;
	{
		...
		FullBlockChain<Elhash> bc(genesis.getBytes(), AccountMap(), tempDirBlockchain.path(), WithExisting::Kill);
	...
	FullBlockChain<Elhash> bc(genesisCopy.getBytes(), AccountMap(), dbPath, WithExisting::Verify);
