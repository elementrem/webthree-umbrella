...
void doStateTests(json_spirit::mValue& _v, bool _fillin)
{
	...
	for (auto& i: _v.get_obj())
	{
		...
		if (importer.m_envInfo.number() >= dev::test::c_testHomesteadBlock)
			output = importer.executeTest(ele::Network::HomesteadTest);
		else
			output = importer.executeTest(ele::Network::FrontierTest);
...
BOOST_AUTO_TEST_CASE(stCreateTest)
{
	for (int i = 1; i < boost::unit_test::framework::master_test_suite().argc; ++i)
	{
		...
		if (arg == "--createtest")
		{
			...
			{
				cnote << "usage: ./testele --createtest <PathToConstructor> <PathToDestiny>\n";
				return;
			}
			try
			{
