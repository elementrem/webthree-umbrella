...
int createRandomTest(std::vector<char*> const& _parameters)
{
	...
	else
	{
		...
		if (testSuite == "VMTests")
		{
			if (checktest)
			{
				dev::ele::VMFactory::setKind(dev::ele::VMKind::JIT);
...
void dev::test::RandomCode::parseTestWithTypes(std::string& _test, std::map<std::string, std::string> const& _varMap)
{
	...
	options.setWeight(dev::ele::Instruction::STOP, 10);		//default 50
	options.setWeight(dev::ele::Instruction::SSTORE, 70);
	options.setWeight(dev::ele::Instruction::CALL, 75);
	options.setWeight(dev::ele::Instruction::CALLCODE, 55);
