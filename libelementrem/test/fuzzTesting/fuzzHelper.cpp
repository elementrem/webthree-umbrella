...
std::string RandomCode::generate(int _maxOpNumber, RandomCodeOptions _options)
{
	...
	for (auto i = 0; i < size; i++)
	{
		...
		dev::ele::InstructionInfo info = dev::ele::instructionInfo((dev::ele::Instruction) opcode);
		...
		else
		{
			...
			code += fillArguments((dev::ele::Instruction) opcode, _options);
		}
...
std::string RandomCode::fillArguments(dev::ele::Instruction _opcode, RandomCodeOptions const& _options)
{
	dev::ele::InstructionInfo info = dev::ele::instructionInfo(_opcode);
	...
	{
		//PUSH1 ... PUSH32
		if (dev::ele::Instruction::PUSH1 <= _opcode && _opcode <= dev::ele::Instruction::PUSH32)
		{
		  code += rndByteSequence(int(_opcode) - int(dev::ele::Instruction::PUSH1) + 1);
		  return code;
		}

		//SWAP1 ... SWAP16 || DUP1 ... DUP16
		bool isSWAP = (dev::ele::Instruction::SWAP1 <= _opcode && _opcode <= dev::ele::Instruction::SWAP16);
		bool isDUP = (dev::ele::Instruction::DUP1 <= _opcode && _opcode <= dev::ele::Instruction::DUP16);

		if (isSWAP || isDUP)
		{
			int times = 0;
			if (isSWAP)
				times = int(_opcode) - int(dev::ele::Instruction::SWAP1) + 2;
			else
			if (isDUP)
				times = int(_opcode) - int(dev::ele::Instruction::DUP1) + 1;
		...
		switch (_opcode)
		{
		case dev::ele::Instruction::CREATE:
		...
		break;
		case dev::ele::Instruction::CALL:
		case dev::ele::Instruction::CALLCODE:
		...
		break;
		case dev::ele::Instruction::SUICIDE: //(SUICIDE address)
			code += getPushCode(toString(_options.getRandomAddress()));
		break;
		case dev::ele::Instruction::RETURN:  //(RETURN memlen1 memlen2)
			code += getPushCode(randUniIntGen() % 128);  //memlen1
			code += getPushCode(randUniIntGen() % 32);  //memlen1
		break;
		default:
...
void RandomCodeOptions::setWeight(dev::ele::Instruction _opCode, int _weight)
{
