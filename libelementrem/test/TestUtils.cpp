...
{
	m_json = loadJsonFromFile(toTestFilePath(getCommandLineArgument("--ele_testfile")));
}

void ParallelFixture::enumerateThreads(std::function<void()> callback) const
{
	size_t threadsCount = std::stoul(getCommandLineArgument("--ele_threads"), nullptr, 10);
...
void BlockChainFixture::enumerateBlockchains(std::function<void(Json::Value const&, dev::ele::BlockChain const&, State state)> callback) const
{
...
void ClientBaseFixture::enumerateClients(std::function<void(Json::Value const&, dev::ele::ClientBase&)> callback) const
{
...
void ParallelClientBaseFixture::enumerateClients(std::function<void(Json::Value const&, dev::ele::ClientBase&)> callback) const
{
	ClientBaseFixture::enumerateClients([this, &callback](Json::Value const& _json, dev::ele::ClientBase& _client) -> void
	{
