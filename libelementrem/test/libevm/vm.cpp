...
ele::OnOpFunc FakeExtVM::simpleTrace()
{

	return [](uint64_t steps, ele::Instruction inst, bigint newMemSize, bigint gasCost, bigint gas, dev::ele::VM* voidVM, dev::ele::ExtVMFace const* voidExt)
	{
		...
		dev::LogOutputStream<ele::VMTraceChannel, false>() << o.str();
		dev::LogOutputStream<ele::VMTraceChannel, false>() << " | " << std::dec << ext.depth << " | " << ext.myAddress << " | #" << steps << " | " << std::hex << std::setw(4) << std::setfill('0') << vm.curPC() << " : " << instructionInfo(inst).name << " | " << std::dec << gas << " | -" << std::dec << gasCost << " | " << newMemSize << "x32" << " ]";

		/*creates json stack trace*/
		if (ele::VMTraceChannel::verbosity <= g_logVerbosity)
		{
			Object o_step;
	...
	for (auto& i: _v.get_obj())
	{
		...
		else
		{
			if (o.count("post") > 0)	// No exceptions expected
			{
				...
				dev::test::FakeExtVM test(ele::EnvInfo{});
...
BOOST_AUTO_TEST_CASE(vmRandom)
{
	...
	for (auto& path: testFiles)
	{
		try
		{
			...
			BOOST_REQUIRE_MESSAGE(s.length() > 0, "Content of " + path.string() + " is empty. Have you cloned the 'tests' repo branch develop and set ELEMENTREM_TEST_PATH to its path?");
