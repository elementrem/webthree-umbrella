...
#include <libelecore/Transaction.h>
	...
	void execute()
	{
		...
		case OperationMode::DecodeTx:
		{
			try
			{
				...
				cout << "  value: " << formatBalance(t.value()) << " (" << t.value() << " mey)" << endl;
				cout << "  nonce: " << t.nonce() << endl;
				cout << "  gas: " << t.gas() << endl;
				cout << "  gas price: " << formatBalance(t.gasPrice()) << " (" << t.gasPrice() << " mey)" << endl;
...
//			<< "    exportbare [ <uuid> , ... ]  Export given keys." << endl
			...
			<< "    createwallet  Create an Elementrem master wallet." << endl
...
//			<< "    recode [ <address>|<uuid>|<file> , ... ]  Decrypt and re-encrypt given keys." << endl
			...
			<< "    --wallet-path <path>  Specify Elementrem wallet path (default: " << KeyManager::defaultPath() << ")" << endl
			...
			<< "    --tx-gasprice <mey>  Specify the gas price for the transaction to be signed." << endl
			<< "    --tx-value <mey>  Specify the value for the transaction to be signed." << endl
