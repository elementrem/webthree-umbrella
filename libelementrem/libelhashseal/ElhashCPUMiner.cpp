...
unsigned ElhashCPUMiner::s_numInstances = 0;
...
ElhashCPUMiner::ElhashCPUMiner(GenericMiner<ElhashProofOfWork>::ConstructionInfo const& _ci):
	GenericMiner<ElhashProofOfWork>(_ci), Worker("miner" + toString(index()))
{
}

ElhashCPUMiner::~ElhashCPUMiner()
{
}

void ElhashCPUMiner::kickOff()
{
	stopWorking();
	startWorking();
}

void ElhashCPUMiner::pause()
{
	stopWorking();
}

void ElhashCPUMiner::workLoop()
{
	auto tid = std::this_thread::get_id();
	static std::mt19937_64 s_eng((utcTime() + std::hash<decltype(tid)>()(tid)));

	uint64_t tryNonce = s_eng();
	elhash_return_value elhashReturn;
	...
	{
		while (!shouldStop() && ElhashAux::computeFull(w.seedHash, true) != 100)
			this_thread::sleep_for(chrono::milliseconds(500));
		dag = ElhashAux::full(w.seedHash, false);
	}
	...
	{
		elhashReturn = elhash_full_compute(dag->full, *(elhash_h256_t*)w.headerHash.data(), tryNonce);
		h256 value = h256((uint8_t*)&elhashReturn.result, h256::ConstructFromPointer);
		if (value <= boundary && submitProof(ElhashProofOfWork::Solution{(h64)(u64)tryNonce, h256((uint8_t*)&elhashReturn.mix_hash, h256::ConstructFromPointer)}))
			break;
		if (!(hashCount % 100))
...
std::string ElhashCPUMiner::platformInfo()
{
