/*
  This file is part of c-elhash.

  c-elhash is free software: you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.

  c-elhash is distributed in the hope that it will be useful,
...
#include <atomic>
#include <sstream>
#include <libelhash/util.h>
#include <libelhash/elhash.h>
#include <libelhash/internal.h>
#include "elhash_cl_miner.h"
#include "elhash_cl_miner_kernel.h"

#define ELHASH_BYTES 32
...
unsigned const elhash_cl_miner::c_defaultLocalWorkSize = 64;
unsigned const elhash_cl_miner::c_defaultGlobalWorkSizeMultiplier = 4096; // * CL_DEFAULT_LOCAL_WORK_SIZE
unsigned const elhash_cl_miner::c_defaultMSPerBatch = 0;

// TODO: If at any point we can use libdevcore in here then we should switch to using a LogChannel
#if defined(_WIN32)
extern "C" __declspec(dllimport) void __stdcall OutputDebugStringA(const char* lpOutputString);
static std::atomic_flag s_logSpin = ATOMIC_FLAG_INIT;
#define ELECL_LOG(_contents) \
	do \
	{ \
		std::stringstream ss; \
		ss << _contents; \
...
#else
#define ELECL_LOG(_contents) cout << "[OPENCL]:" << _contents << endl
#endif
// Types of OpenCL devices we are interested in
#define ELECL_QUERIED_DEVICE_TYPES (CL_DEVICE_TYPE_GPU | CL_DEVICE_TYPE_ACCELERATOR)
...
elhash_cl_miner::search_hook::~search_hook() {}

elhash_cl_miner::elhash_cl_miner()
:	m_openclOnePointOne()
{
}
...
std::vector<cl::Platform> elhash_cl_miner::getPlatforms()
{
	...
	catch(cl::Error const& err)
	{
		...
		if (err.err() == CL_PLATFORM_NOT_FOUND_KHR)
			ELECL_LOG("No OpenCL platforms found");
		else
#endif
			throw err;
	}
...
string elhash_cl_miner::platform_info(unsigned _platformId, unsigned _deviceId)
{
	...
	if (devices.empty())
	{
		ELECL_LOG("No OpenCL devices found.");
		return {};
	}
...
std::vector<cl::Device> elhash_cl_miner::getDevices(std::vector<cl::Platform> const& _platforms, unsigned _platformId)
{
	...
	try
	{
		_platforms[platform_num].getDevices(
			s_allowCPU ? CL_DEVICE_TYPE_ALL : ELECL_QUERIED_DEVICE_TYPES,
			&devices
		);
	}
	catch (cl::Error const& err)
...
unsigned elhash_cl_miner::getNumPlatforms()
{
	vector<cl::Platform> platforms = getPlatforms();
	if (platforms.empty())
		return 0;
...
unsigned elhash_cl_miner::getNumDevices(unsigned _platformId)
{
	vector<cl::Platform> platforms = getPlatforms();
	if (platforms.empty())
		return 0;
...
bool elhash_cl_miner::configureGPU(
...
)
{
	...
	uint64_t dagSize = elhash_get_datasize(_currentBlock);
...
bool elhash_cl_miner::s_allowCPU = false;
unsigned elhash_cl_miner::s_extraRequiredGPUMem;
unsigned elhash_cl_miner::s_msPerBatch = elhash_cl_miner::c_defaultMSPerBatch;
unsigned elhash_cl_miner::s_workgroupSize = elhash_cl_miner::c_defaultLocalWorkSize;
unsigned elhash_cl_miner::s_initialGlobalWorkSize = elhash_cl_miner::c_defaultGlobalWorkSizeMultiplier * elhash_cl_miner::c_defaultLocalWorkSize;

bool elhash_cl_miner::searchForAllDevices(function<bool(cl::Device const&)> _callback)
{
	vector<cl::Platform> platforms = getPlatforms();
	if (platforms.empty())
		return false;
...
bool elhash_cl_miner::searchForAllDevices(unsigned _platformId, function<bool(cl::Device const&)> _callback)
{
	vector<cl::Platform> platforms = getPlatforms();
	if (platforms.empty())
		return false;
...
void elhash_cl_miner::doForAllDevices(function<void(cl::Device const&)> _callback)
{
	vector<cl::Platform> platforms = getPlatforms();
	if (platforms.empty())
		return;
...
void elhash_cl_miner::doForAllDevices(unsigned _platformId, function<void(cl::Device const&)> _callback)
{
	vector<cl::Platform> platforms = getPlatforms();
	if (platforms.empty())
		return;
...
void elhash_cl_miner::listDevices()
{
...
void elhash_cl_miner::finish()
{
	if (m_queue())
		m_queue.finish();
}

bool elhash_cl_miner::init(
	uint8_t const* _dag,
	uint64_t _dagSize,
	unsigned _platformId,
	unsigned _deviceId
)
{
	...
	try
	{
		...
		ELECL_LOG("Using platform: " << platforms[_platformId].getInfo<CL_PLATFORM_NAME>().c_str());
		...
		string device_version = device.getInfo<CL_DEVICE_VERSION>();
		ELECL_LOG("Using device: " << device.getInfo<CL_DEVICE_NAME>().c_str() << "(" << device_version.c_str() << ")");

		if (strncmp("OpenCL 1.0", device_version.c_str(), 10) == 0)
		{
			ELECL_LOG("OpenCL 1.0 is not supported.");
			return false;
		}
		...
		// patch source code
		// note: ELHASH_CL_MINER_KERNEL is simply elhash_cl_miner_kernel.cl compiled
		// into a byte array by bin2h.cmake. There is no need to load the file by hand in runtime
		string code(ELHASH_CL_MINER_KERNEL, ELHASH_CL_MINER_KERNEL + ELHASH_CL_MINER_KERNEL_SIZE);
		addDefinition(code, "GROUP_SIZE", s_workgroupSize);
		addDefinition(code, "DAG_SIZE", (unsigned)(_dagSize / ELHASH_MIX_BYTES));
		addDefinition(code, "ACCESSES", ELHASH_ACCESSES);
		...
		try
		{
			...
			ELECL_LOG("Printing program log");
			ELECL_LOG(program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(device).c_str());
		}
		...
		{
			m_dagChunksCount = 1;
			ELECL_LOG("Creating one big buffer for the DAG");
			m_dagChunks.push_back(cl::Buffer(m_context, CL_MEM_READ_ONLY, _dagSize));
			ELECL_LOG("Loading single big chunk kernels");
			m_hashKernel = cl::Kernel(program, "elhash_hash");
			m_searchKernel = cl::Kernel(program, "elhash_search");
			ELECL_LOG("Mapping one big chunk.");
			m_queue.enqueueWriteBuffer(m_dagChunks[0], CL_TRUE, 0, _dagSize, _dag);
		}
		catch (cl::Error const& err)
		{
			ELECL_LOG("Allocating/mapping single buffer failed with: " << err.what() << "(" << err.err() << "). GPU can't allocate the DAG in a single chunk. Bailing.");
			return false;
			...
			if (errCode != CL_INVALID_BUFFER_SIZE || errCode != CL_MEM_OBJECT_ALLOCATION_FAILURE)
				ELECL_LOG("Allocating/mapping single buffer failed with: " << err.what() << "(" << errCode << ")");
			cl_ulong result;
			...
			for (unsigned i = 0; i < m_dagChunksCount; i++)
			{
				...
				ELECL_LOG("Creating buffer for chunk " << i);
			...
			ELECL_LOG("Loading chunk kernels");
			m_hashKernel = cl::Kernel(program, "elhash_hash_chunks");
			m_searchKernel = cl::Kernel(program, "elhash_search_chunks");
			// TODO Note: If we ever change to _dagChunksNum other than 4, then the size would need recalculation
			void* dag_ptr[4];
			for (unsigned i = 0; i < m_dagChunksCount; i++)
			{
				ELECL_LOG("Mapping chunk " << i);
		...
		ELECL_LOG("Creating buffer for header.");
		m_header = cl::Buffer(m_context, CL_MEM_READ_ONLY, 32);

		// create mining buffers
		for (unsigned i = 0; i != c_bufferCount; ++i)
		{
			ELECL_LOG("Creating mining buffer " << i);
	...
	{
		ELECL_LOG(err.what() << "(" << err.err() << ")");
		return false;
	}
	return true;
}

void elhash_cl_miner::search(uint8_t const* header, uint64_t target, search_hook& hook)
{
	try
	{
