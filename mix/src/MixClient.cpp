...
{
	dev::ele::Elhash::init();
	dev::ele::BasicAuthority::init();
	dev::ele::NoProof::init();
...
void MixClient::resetState(unordered_map<Address, Account> const& _accounts, Secret const& _miner)
{
	...
	accountState.init();
	dev::ele::commit(_accounts, accountState);
...
void MixClient::executeTransaction(Transaction const& _t, Block& _block, bool _call, bool _gasAuto, Secret const& _secret)
{
	...
	if (!_call && d.excepted == TransactionException::None)
	{
		...
		ele::ExecutionResult const& er = _block.execute(envInfo.lastHashes(), t);
...
pair<h256, Address> MixClient::submitTransaction(ele::TransactionSkeleton const& _ts, Secret const& _secret, bool _gasAuto)
{
...
{
	(void)_blockNumber;
	Block block = asOf(ele::PendingBlock);
...
dev::ele::ExecutionResult MixClient::create(
	Address const& _from,
	u256 _value,
	bytes const& _data,
	u256 _gas,
...
ele::BlockHeader MixClient::blockInfo() const
{
	ReadGuard l(x_state);
