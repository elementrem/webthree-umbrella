...
void ClientModel::init(QString _dbpath)
{
	try
	{
		...
		m_eleAccounts = make_shared<FixedAccountHolder>([=](){return m_client.get();}, vector<KeyPair>());
		auto eleFace = new Web3Server(*m_client.get(), *m_eleAccounts.get());
		m_web3Server.reset(new ModularServer<rpc::EleFace, rpc::DBFace, rpc::Web3Face>(eleFace, new rpc::MemoryDB(), new rpc::Web3()));
		m_rpcConnectorId = m_web3Server->addConnector(new RpcConnector());
		connect(eleFace, &Web3Server::newTransaction, this, [=]() {
...
void ClientModel::addAccount(QString const& _secret)
{
	try
	{
		...
		m_eleAccounts->setAccounts(m_accountsSecret);
	}
	catch (...)
	{
		manageException();
...
void ClientModel::setupScenario(QVariantMap _scenario)
{
	try
	{
		...
		for (auto const& b: stateAccounts)
		{
			...
			m_accounts[address] = Account(0, qvariant_cast<QElement*>(account.value("balance"))->toU256Mey(), Account::NormalCreation);
		}
		...
		for (auto const& c: stateContracts)
		{
			...
			Account account(0, qvariant_cast<QElement*>(contract.value("balance"))->toU256Mey(), Account::ContractConception);
...
TransactionSettings ClientModel::transaction(QVariant const& _tr) const
{
	...
	u256 value = (qvariant_cast<QElement*>(transaction.value("value")))->toU256Mey();
	u256 gasPrice = (qvariant_cast<QElement*>(transaction.value("gasPrice")))->toU256Mey();
		...
		try
		{
			for (TransactionSettings const& transaction: _sequence)
			{
				...
				if (transaction.functionId.isEmpty() || transaction.functionId == ctrInstance.first)
				{
					...
					ele::LinkerObject object = m_codeModel->contract(ctrInstance.first)->linkerObject();
...
void ClientModel::showDebuggerForTransaction(ExecutionResult const& _t, QString const& _label)
{
	try
	{
		...
		for (MachineCode const& code: _t.executionCode)
		{
			...
			{
				ele::AssemblyItems assemblyItems = !_t.isConstructor() ? compilerRes->assemblyItems() : compilerRes->constructorAssemblyItems();
...
RecordLogEntry* ClientModel::lastBlock() const
{
	try
	{
		ele::BlockHeader blockInfo = m_client->blockInfo();
		stringstream strGas;
...
void ClientModel::onNewTransaction(RecordLogEntry::TxSource _source)
{
	try
	{
		...
		for (auto const& ctr : m_contractAddresses)
		{
			u256 mey = m_client->balanceAt(ctr.second, PendingBlock);
			...
			contractsStorage.insert(contractAddressIter->second + " - " + QString::fromStdString(ctr.second.hex()) + " - " + QElement(mey, QElement::Mey).format(), sto);
		}
		for (auto const& account : m_accounts)
		{
			u256 mey = m_client->balanceAt(account.first, PendingBlock);
			accountBalances.insert("0x" + QString::fromStdString(account.first.hex()),  QElement(mey, QElement::Mey).format());
		}
