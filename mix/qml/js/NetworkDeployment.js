 ...
 */
.import org.elementrem.qml.QSolidityType 1.0 as QSolidityType
...
function checkPathCreationCost(eleUrl, callBack)
{
	var dappUrl = formatAppUrl(eleUrl);
	checkElePath(dappUrl, true, function(success, cause) {
		if (!success)
		{
			switch (cause)
			{
...
function executeTr(blockIndex, trIndex, state, ctrAddresses, trHashes, callBack)
{
	...
	else
	{
		...
		{
			codeModel.linkLibraries(contractFromToken(tr.contractId), deployedLibraries)
...
function copyFiles(path, target)
{
	var files = fileIo.files(path)
	for (var i = 0; i < files.length; i++)
	{
		var doc = files[i];
		doc = projectModel.file(doc)
		if (doc.isContract)
			continue;
		if (doc.isHtml) {
			...
			fileIo.writeFile(target + doc.fileName, html);
		}
		else
			fileIo.copyFile(doc.path, target + doc.fileName);
	}

	var dirs = fileIo.directories(path)
	for (var i = 0; i < dirs.length; i++)
	{
		if (dirs[i].fileName !== ".." && dirs[i].fileName !== ".")
		{
...
function packageDapp(addresses)
{
	...
	if (deploymentDialog.packageStep.packageDir !== "")
		...
		deploymentDir = projectPath + "/" + deploymentDialog.packageStep.folderName + "/"
	...
	for (var c in addresses)
	{
		var name = contractFromToken(c)
		var contractAccessor = "contracts[\"" + c + "\"]";

		deploymentJs += contractAccessor + " = {\n" +
				"\tinterface: " + codeModel.contracts[name].contractInterface + ",\n" +
				"\taddress: \"" + addresses[c] + "\"\n" +
				"};\n" +
				contractAccessor + ".contractClass = web3.ele.contract(" + contractAccessor + ".interface);\n" +
				contractAccessor + ".contract = " + contractAccessor + ".contractClass.at(" + contractAccessor + ".address);\n";
		if (name !== c)
		{
			deploymentJs += "if (!ctrAddresses[\"" + name + "\"])" +
								"{ ctrAddresses[\"" + name + "\"] = [] }" +
							"ctrAddresses[\"" + name + "\"].push(\"" + addresses[c] + "\")"
		}
	}

	fileIo.writeFile(wwwFolder + "deployment.js", deploymentJs);
...
{
	deploymentGasPrice = gasPrice
	deploymentStepChanged(qsTr("Registering application on the Elementrem network ..."));
	checkElePath(url, false, function (success) {
		if (!success)
			return;
...
function checkElePath(dappUrl, checkOnly, callBack)
{
	if (dappUrl.length === 1)
	{
		// convenient for dev purpose, should not be possible in normal env.
		if (!checkOnly)
			reserve(deploymentDialog.registerStep.ele, function() {
				registerContentHash(deploymentDialog.registerStep.ele, callBack); // we directly create a dapp under the root registrar.
			});
		else
			callBack(true);
	}
	else
	{
		...
		requests.push({
						  ...
						  jsonrpc: "2.0",
						  method: "ele_call",
						  params: [ { "gas": "0xffff", "from": deploymentDialog.worker.currentAccount,  "to": '0x' + deploymentDialog.registerStep.ele, "data": "0xe1fa8e84" + str }, "pending" ],
						  id: jsonRpcRequestId++
					  });
...
function registerContentHash(registrar, callBack)
{
	...
	deploymentStepChanged(txt);
	console.log(txt);
	console.log("register url " + projectModel.projectTitle)
	...
	requests.push({
					  ...
					  jsonrpc: "2.0",
					  method: "ele_sendTransaction",
					  params: [ { "from": deploymentDialog.worker.currentAccount, "gasPrice": deploymentGasPrice, "gas": "0x" + gasCost, "to": '0x' + registrar, "data": "0xc3d014d6" + paramTitle } ],
					  id: jsonRpcRequestId++
				  });
	rpcCall(requests, function (httpRequest, response) {
...
function registerToUrlHint(url, gasPrice, callback)
{
	...
	urlHintAddress(function(urlHint){
		...
		requests.push({
						  ...
						  jsonrpc: "2.0",
						  method: "ele_sendTransaction",
						  params: [ {  "to": '0x' + urlHint, "gasPrice": deploymentGasPrice, "from": deploymentDialog.worker.currentAccount, "gas": "0x" + gasCost, "data": "0x584e86ad" + paramUrlHttp } ],
						  id: jsonRpcRequestId++
					  });
...
function urlHintAddress(callBack)
{
	...
	requests.push({
					  ...
					  jsonrpc: "2.0",
					  method: "ele_call",
					  params: [ {  "to": '0x' + deploymentDialog.registerStep.ele, "from": deploymentDialog.worker.currentAccount, "data": "0x3b3b57de" + urlHint }, "pending" ],
					  id: jsonRpcRequestId++
				  });
...
function formatAppUrl(url)
{
	...
	if (url.toLowerCase().indexOf("ele://") === 0)
		url = url.substring(6);
