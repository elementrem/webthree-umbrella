...
Dialog {
	...
	onVisibleChanged:
	{
		...
		{
			addressText.text = ""
			balance.value = QElementHelper.createElement("0", QElement.Mey)
			...
			RowLayout
			{
				...
				DefaultButton {
					...
					onClicked: {
						...
						var bal = balance.value.toMey()
						var balanceStr = "0"
						if (bal !== null)
							balanceStr = bal.value()
						var ac = projectModel.stateListModel.newAccount(balanceStr, QElement.Mey, newAddressWin.secret.toLowerCase(), nickNameInput.text)
