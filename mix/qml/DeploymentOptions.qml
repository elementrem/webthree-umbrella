...
Rectangle
{
	...
	Column
	{
		...
		DefaultLabel
		{
			...
			visible: false
			color: "red"
			text: qsTr("Unable to contact Elementrem node on ") + appSettings.nodeAddress
		}
		...
		RowLayout
		{
			...
			function setCost()
			{
				...
				{
					var element = QElementHelper.createBigInt(deployStep.gasUsed);
					var gasPriceMey = gasPriceConf.gasPrice.toMey();
					if (gasPriceMey !== null)
					{
						var gasTotal = element.multiply(gasPriceMey);
						if (gasTotal !== null)
							gasToUseInput.value = QElementHelper.createElement(gasTotal.value(), QElement.Mey, parent);
					}
				}
			}
	...
	Rectangle
	{
		...
		DefaultButton
		{
			...
			function deploy()
			{
				...
				NetworkDeploymentCode.deployContracts(deployStep.gasByTx, gasPriceConf.toHexMey(), function(addresses, trHashes)
				{
