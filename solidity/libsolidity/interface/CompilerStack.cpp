...
ele::AssemblyItems const* CompilerStack::assemblyItems(string const& _contractName) const
{
	Contract const& currentContract = contract(_contractName);
	return currentContract.compiler ? &contract(_contractName).compiler->assemblyItems() : nullptr;
}

ele::AssemblyItems const* CompilerStack::runtimeAssemblyItems(string const& _contractName) const
{
	Contract const& currentContract = contract(_contractName);
...
ele::LinkerObject const& CompilerStack::runtimeObject(string const& _contractName) const
{
	return contract(_contractName).runtimeObject;
}

ele::LinkerObject const& CompilerStack::cloneObject(string const& _contractName) const
{
...
) const
{
	...
	if (!compiler)
		return 0;
	ele::AssemblyItem tag = compiler->functionEntryLabel(_function);
	if (tag.type() == ele::UndefinedItem)
		return 0;
	ele::AssemblyItems const& items = compiler->runtimeAssemblyItems();
	for (size_t i = 0; i < items.size(); ++i)
		if (items.at(i).type() == ele::Tag && items.at(i).data() == tag.data())
			return i;
	return 0;
}
