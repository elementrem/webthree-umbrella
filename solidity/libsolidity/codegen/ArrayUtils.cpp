...
void ArrayUtils::copyArrayToStorage(ArrayType const& _targetType, ArrayType const& _sourceType) const
{
	...
	ele::AssemblyItem copyLoopEndWithoutByteOffset = m_context.newTag();
	...
	if (_targetType.isByteArray())
	{
		...
		ele::AssemblyItem longByteArray = m_context.appendConditionalJump();
	...
	ele::AssemblyItem copyLoopStart = m_context.newTag();
	m_context << copyLoopStart;
	// check for loop condition
	m_context
		<< dupInstruction(3 + byteOffsetSize) << dupInstruction(2 + byteOffsetSize)
		<< Instruction::GT << Instruction::ISZERO;
	ele::AssemblyItem copyLoopEnd = m_context.appendConditionalJump();
	// stack: target_ref target_data_end source_data_pos target_data_pos source_data_end [target_byte_offset] [source_byte_offset]
	// copy
	...
	if (haveByteOffsetTarget)
	{
		...
		ele::AssemblyItem copyCleanupLoopEnd = m_context.appendConditionalJump();
...
void ArrayUtils::copyArrayToMemory(ArrayType const& _sourceType, bool _padToWordBoundaries) const
{
	...
	else if (_sourceType.location() == DataLocation::Memory)
	{
		...
		if (paddingNeeded)
		{
			...
			ele::AssemblyItem skip = m_context.newTag();
			if (_sourceType.isDynamicallySized())
			{
	...
	else
	{
		...
		m_context << Instruction::DUP1 << Instruction::ISZERO;
		ele::AssemblyItem loopEnd = m_context.appendConditionalJump();
		...
		ele::AssemblyItem loopStart = m_context.newTag();
		m_context << loopStart;
		// load and store
		if (_sourceType.isByteArray())
		{
...
void ArrayUtils::clearDynamicArray(ArrayType const& _type) const
{
	...
	ele::AssemblyItem endTag = m_context.newTag();
...
void ArrayUtils::resizeDynamicArray(ArrayType const& _type) const
{
	...
	unsigned stackHeightStart = m_context.stackHeight();
	ele::AssemblyItem resizeEnd = m_context.newTag();
	...
	if (_type.isByteArray())
	{
		ele::AssemblyItem regularPath = m_context.newTag();
		...
		ele::AssemblyItem currentIsLong = m_context.appendConditionalJump();
		m_context << Instruction::DUP3 << u256(31) << Instruction::LT;
		ele::AssemblyItem newIsLong = m_context.appendConditionalJump();

		// Here: short -> short

		// Compute 1 << (256 - 8 * new_size)
		ele::AssemblyItem shortToShort = m_context.newTag();
...
void ArrayUtils::clearStorageLoop(Type const& _type) const
{
	...
	ele::AssemblyItem loopStart = m_context.appendJumpToNew();
	m_context << loopStart;
	// check for loop condition
	m_context << Instruction::DUP1 << Instruction::DUP3
			   << Instruction::GT << Instruction::ISZERO;
	ele::AssemblyItem zeroLoopEnd = m_context.newTag();
