...
public:
	...
	void setCompiledContracts(std::map<ContractDefinition const*, ele::Assembly const*> const& _contracts) { m_compiledContracts = _contracts; }
	ele::Assembly const& compiledContract(ContractDefinition const& _contract) const;
	...
	ele::AssemblyItem functionEntryLabel(Declaration const& _declaration);
	/// @returns the entry label of the given function. Might return an AssemblyItem of type
	/// UndefinedItem if it does not exist yet.
	ele::AssemblyItem functionEntryLabelIfExists(Declaration const& _declaration) const;
	void setInheritanceHierarchy(std::vector<ContractDefinition const*> const& _hierarchy) { m_inheritanceHierarchy = _hierarchy; }
	/// @returns the entry label of the given function and takes overrides into account.
	ele::AssemblyItem virtualFunctionEntryLabel(FunctionDefinition const& _function);
	/// @returns the entry label of a function that overrides the given declaration from the most derived class just
	/// above _base in the current inheritance hierarchy.
	ele::AssemblyItem superFunctionEntryLabel(FunctionDefinition const& _function, ContractDefinition const& _base);
	...
	ele::AssemblyItem appendConditionalJump() { return m_asm.appendJumpI().tag(); }
	/// Appends a JUMPI instruction to @a _tag
	CompilerContext& appendConditionalJumpTo(ele::AssemblyItem const& _tag) { m_asm.appendJumpI(_tag); return *this; }
	/// Appends a JUMP to a new tag and @returns the tag
	ele::AssemblyItem appendJumpToNew() { return m_asm.appendJump().tag(); }
	/// Appends a JUMP to a tag already on the stack
	CompilerContext&  appendJump(ele::AssemblyItem::JumpType _jumpType = ele::AssemblyItem::JumpType::Ordinary);
	/// Returns an "ErrorTag"
	ele::AssemblyItem errorTag() { return m_asm.errorTag(); }
	/// Appends a JUMP to a specific tag
	CompilerContext& appendJumpTo(ele::AssemblyItem const& _tag) { m_asm.appendJump(_tag); return *this; }
	/// Appends pushing of a new tag and @returns the new tag.
	ele::AssemblyItem pushNewTag() { return m_asm.append(m_asm.newPushTag()).tag(); }
	/// @returns a new tag without pushing any opcodes or data
	ele::AssemblyItem newTag() { return m_asm.newTag(); }
	/// Adds a subroutine to the code (in the data section) and pushes its size (via a tag)
	/// on the stack. @returns the assembly item corresponding to the pushed subroutine, i.e. its offset.
	ele::AssemblyItem addSubroutine(ele::Assembly const& _assembly) { return m_asm.appendSubSize(_assembly); }
	/// Pushes the size of the final program
	void appendProgramSize() { return m_asm.appendProgramSize(); }
	/// Adds data to the data section, pushes a reference to the stack
	ele::AssemblyItem appendData(bytes const& _data) { return m_asm.append(_data); }
	...
	CompilerContext& operator<<(ele::AssemblyItem const& _item) { m_asm.append(_item); return *this; }
	...
	ele::LinkerObject const& assembledObject() { return m_asm.assemble(); }
	ele::LinkerObject const& assembledRuntimeObject(size_t _subIndex) { return m_asm.sub(_subIndex).assemble(); }
...
private:
	...
	ele::AssemblyItem virtualFunctionEntryLabel(
	...
	struct FunctionCompilationQueue
	{
		...
		ele::AssemblyItem entryLabel(Declaration const& _declaration, CompilerContext& _context);
		/// @returns the entry label of the given function. Might return an AssemblyItem of type
		/// UndefinedItem if it does not exist yet.
		ele::AssemblyItem entryLabelIfExists(Declaration const& _declaration) const;
		...
		std::map<Declaration const*, ele::AssemblyItem> m_entryLabels;
	...
	std::map<ContractDefinition const*, ele::Assembly const*> m_compiledContracts;
