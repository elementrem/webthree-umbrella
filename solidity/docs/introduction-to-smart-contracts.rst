...
Element (in "Mey" to be exact) which can be modified by sending transactions that
include Element.
...
It can include binary data (its payload) and Element.
...
Contracts can call other contracts or send Element to non-contract
accounts by the means of message calls. Message calls are similar
to transactions, in that they have a source, a target, data payload,
Element, gas and return data. In fact, every transaction consists of
...
The remaining Element stored at that address is sent to a designated
