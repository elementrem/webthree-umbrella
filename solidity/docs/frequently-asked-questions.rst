...
Solidity is the DEV-created (i.e. Elementrem Foundation-created),
Javascript-inspired language that can be used to create smart contracts
on the Elementrem blockchain. There are other
...
resulting output can be cut/pasted into a gele console to deploy them to the
Elementrem blockchain.
...
Probably the fastest way is the `online compiler <https://elementrem.github.io/browser-solidity/>`_.

You can also use the ``solc`` binary which comes with cpp-elementrem to compile
...
if Element is sent to removed contracts, the Element will be forever lost.

If you want to deactivate your contracts, it is preferable to **disable** them by changing some
internal state which causes all functions to throw. This will make it impossible
to use the contract and element sent to the contract will be returned automatically.
...
Store Element in a contract
=========================

The trick is to create the contract with ``{from:someaddress, value: web3.toMey(3,"element")...}``
...
gele or any other web3.js environment.

"non-constant" functions (those lacking the ``constant`` specifier) must be called
with ``c.someMethod.sendTransaction({from:ele.accounts[x], gas: 1000000});``
...
datastructure on top of it, for example the `iterable mapping <https://github.com/elementrem/dapp-bin/blob/master/library/iterable_mapping.sol>`_.
...
from web3 (inside gele or a web application). The main advantage of events
...
a way to pull out Element from a contract.

If the contract is not meant to receive Element with simple transfers, you
should implement the fallback function as

``function() { throw; }``

this will cause all transactions to this contract that do not call an
existing function to be reverted, so that all Element is sent back.
...
For examples, see the `features.sol <https://github.com/elementrem/dapp-bin/blob/master/library/features.sol>`_.
...
What happens if you send element along with a function call to a contract?
========================================================================

It gets added to the total balance of the contract, just like when you send element when creating a contract.
...
block explorer do not show Element sent between contracts correctly.
...
Can a regular (i.e. non-contract) elementrem account be closed permanently like a contract can?
=============================================================================================
...
How do I initialize a contract with only a specific amount of mey?
==================================================================
...
In this example::
    ...
    contract A {
        ...
        function test() {
            child = (new B).value(10)(); //construct a new B with 10 mey
        }
    }
...
What could have happened if an account has storage value(s) but no code?  Example: http://test.element.camp/account/5f740b3a43fbb99724ce93a879805f4dc89178b5
==========================================================================================================================================================
...
If you want to send 20 Element from a contract to the address ``x``, you use ``x.send(20 element);``.
Here, ``x`` can be a plain address or a contract. If the contract already explicitly defines
a function ``send`` (and thus overwrites the special function), you can use ``address(x).send(20 element);``.
