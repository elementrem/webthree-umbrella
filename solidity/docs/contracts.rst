...
API, this is done as follows::
    ...
    var MyContract = web3.ele.contract(abiArray);
    // deploy new contract
...
Element (witout data).  In such a context, there is very little gas available to
...
::
    ...
    // This contract rejects any Element sent to it. It is good
    // practise to include such a function for every contract
    // in order not to lose Element.
    contract Rejector {
        function() { throw; }
    }
...
::
    ...
    var ClientReceipt = web3.ele.contract(abi);
...
- `Javascript documentation <https://github.com/elementrem/wiki/wiki/JavaScript-API#contract-events>`_
