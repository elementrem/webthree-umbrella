...
::
    ...
    solc github.com/elementrem/dapp-bin/=/usr/local/lib/dapp-bin/ =/usr/local/lib/fallback file.sol

This essentially instructs the compiler to search for anything starting with
``github.com/elementrem/dapp-bin/`` under ``/usr/local/lib/dapp-bin`` and if it does not
...
* If you do **not** want your contracts to receive element when called via ``send``, you can add a throwing fallback function ``function() { throw; }``.
...
- If a contract receives Element (without a function being called), the fallback function is executed. The contract can only rely
...
- If you want to send element using ``address.send``, there are certain details to be aware of:
  ...
  3. Sending Element can also fail because the recipient goes out of gas (either explicitly by using ``throw`` or
...
- ``msg.value`` (``uint``): number of mey sent with the message
- ``now`` (``uint``): current block timestamp (alias for ``block.timestamp``)
- ``tx.gasprice`` (``uint``): gas price of the transaction
- ``tx.origin`` (``address``): sender of the transaction (full call chain)
- ``sha3(...) returns (bytes32)``: compute the Elementrem-SHA-3 (KECCAK-256) hash of the (tightly packed) arguments
...
- ``<address>.balance`` (``uint256``): balance of the address in Mey
- ``<address>.send(uint256 amount) returns (bool)``: send given amount of Mey to address, returns ``false`` on failure
