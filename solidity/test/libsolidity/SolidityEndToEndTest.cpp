...
BOOST_AUTO_TEST_CASE(send_element)
{
	char const* sourceCode = "contract test {\n"
...
BOOST_AUTO_TEST_CASE(version_stamp_for_libraries)
{
	...
	BOOST_CHECK_EQUAL(runtimeCode[0], int(ele::Instruction::PUSH6)); // might change once we switch to 1.x.x
	BOOST_CHECK_EQUAL(runtimeCode[1], 3); // might change once we switch away from x.3.x
	BOOST_CHECK_EQUAL(runtimeCode[7], int(ele::Instruction::POP));
}
...
BOOST_AUTO_TEST_CASE(reject_element_sent_to_library)
{
	char const* sourceCode = R"(
		library lib {}
		contract c {
