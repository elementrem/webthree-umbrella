...
public:
	...
	{
		ele::NoProof::init();
		m_sealEngine.reset(ele::ChainParams().createSealEngine());
		if (g_logVerbosity != -1)
			g_logVerbosity = 0;
...
protected:
	void sendMessage(bytes const& _data, bool _isCreation, u256 const& _value = 0)
	{
		...
		ele::Executive executive(m_state, m_envInfo, m_sealEngine.get());
		ele::ExecutionResult res;
		executive.setResultRecipient(res);
		ele::Transaction t =
			_isCreation ?
				ele::Transaction(_value, m_gasPrice, m_gas, _data, 0, KeyPair::create().sec()) :
				ele::Transaction(_value, m_gasPrice, m_gas, m_contractAddress, _data, 0, KeyPair::create().sec());
		...
		BOOST_REQUIRE(executive.go(/* DEBUG ele::Executive::simpleTrace() */));
	...
	std::unique_ptr<ele::SealEngineFace> m_sealEngine;
	...
	ele::State m_state;
	u256 const m_gasPrice = 100 * ele::szabo;
