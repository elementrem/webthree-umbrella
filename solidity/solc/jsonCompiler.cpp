...
Json::Value estimateGas(CompilerStack const& _compiler, string const& _contract)
{
	...
	if (!_compiler.assemblyItems(_contract) && !_compiler.runtimeAssemblyItems(_contract))
		return gasEstimates;
	if (ele::AssemblyItems const* items = _compiler.assemblyItems(_contract))
	{
		...
		creationGas[1] = gasToJson(bytecodeSize * ele::GasCosts::createDataGas);
		gasEstimates["creation"] = creationGas;
	}
	if (ele::AssemblyItems const* items = _compiler.runtimeAssemblyItems(_contract))
	{
