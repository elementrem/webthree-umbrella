...
void CommandLineInterface::handleGasEstimation(string const& _contract)
{
	...
	if (ele::AssemblyItems const* items = m_compiler->assemblyItems(_contract))
	{
		Gas gas = GasEstimator::functionalEstimation(*items);
		u256 bytecodeSize(m_compiler->runtimeObject(_contract).bytecode.size());
		cout << "construction:" << endl;
		cout << "   " << gas << " + " << (bytecodeSize * ele::GasCosts::createDataGas) << " = ";
		gas += bytecodeSize * ele::GasCosts::createDataGas;
		cout << gas << endl;
	}
	if (ele::AssemblyItems const* items = m_compiler->runtimeAssemblyItems(_contract))
	{
...
void CommandLineInterface::handleAst(string const& _argStr)
{
	...
	if (m_args.count(_argStr))
	{
		...
		map<ASTNode const*, ele::GasMeter::GasConsumption> gasCosts;
...
void CommandLineInterface::outputAssembly()
{
	for (auto const& src: m_sourceCodes)
	{
		...
		ele::Assembly assembly = m_assemblyStacks[src.first].assemble();
