...
{
	return m_web3->elementrem()->call(_to, _data).output;
}
...
{
	return Address(m_web3->elementrem()->chainParams().otherParams.at("registrar"));
}
...
void Support::hintURL(h256 const& _content, std::string const& _url, Secret const& _s) const
{
	...
	t.to = urlHint();
	m_web3->elementrem()->submitTransaction(t, _s);
}
