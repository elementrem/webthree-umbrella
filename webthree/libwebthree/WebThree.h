...
public:
	...
	/// elementrem() may be safely static_cast()ed to a ele::Client*.
	WebThreeDirect(
		std::string const& _clientVersion,
		std::string const& _dbPath,
		ele::ChainParams const& _params,
		WithExisting _we = WithExisting::Trust,
		std::set<std::string> const& _interfaces = {"ele", "shh", "bzz"},
	...
	ele::Client* elementrem() const { if (!m_elementrem) BOOST_THROW_EXCEPTION(InterfaceNotSupported("ele")); return m_elementrem.get(); }
	...
	u256 networkId() const override { return m_elementrem.get()->networkId(); }
...
private:
	...
	std::unique_ptr<ele::Client> m_elementrem;		///< Client for Elementrem ("ele") protocol.
...
// TODO, probably move into ele:

class ElementremSlave: public ele::Interface
{
public:
	ElementremSlave(RPCSlave*) {}

	// TODO: implement all of the virtuals with the RLPClient link.
};

class ElementremMaster
{
public:
	ElementremMaster(RPCMaster*) {}
...
public:
	...
	ele::Interface* elementrem() const { if (!m_elementrem) BOOST_THROW_EXCEPTION(InterfaceNotSupported("ele")); return m_elementrem; }
	...
	// Peer network stuff - forward through RPCSlave, probably with P2PNetworkSlave/Master classes like Whisper & Elementrem.
	...
	void connect(std::string const& _seedHost, unsigned short _port = 30707);
...
private:
	ElementremSlave* m_elementrem = nullptr;
