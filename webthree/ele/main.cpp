...
#include <libelementrem/All.h>
#include <libelementrem/BlockChainSync.h>
...
void help()
{
	...
	cout
		...
		<< "    -e,--element-price <n>  Set the element price in the reference unit, e.g. ¢ (default: 30.679)." << endl
		<< "    -P,--priority <0 - 100>  Set the default priority percentage (%) of a transaction (default: 50)." << endl*/
		<< "    --ask <mey>  Set the minimum ask gas price under which no transaction will be mined (default " << toString(DefaultGasPrice) << " )." << endl
		<< "    --bid <mey>  Set the bid gas price to pay for transactions (default " << toString(DefaultGasPrice) << " )." << endl
		...
		<< "    --bootstrap  Connect to the default Elementrem peer servers (default unless --no-discovery used)." << endl
		<< "    --no-bootstrap  Do not connect to the default Elementrem peer servers (default only when --no-discovery is used)." << endl
		...
		<< "    --listen <port>  Listen on the given port for incoming connections (default: 30707)." << endl
		<< "    -r,--remote <host>(:<port>)  Connect to the given remote host (default: none)." << endl
		<< "    --port <port>  Connect to the given remote port (default: 30707)." << endl
...
string eleCredits(bool _interactive = false)
{
...
void version()
{
	cout << "ele version " << dev::Version << endl;
	cout << "ele network protocol version: " << dev::ele::c_protocolVersion << endl;
	cout << "Client database version: " << dev::ele::c_databaseVersion << endl;
...
string pretty(h160 _a, dev::ele::State const& _st)
{
	string ns;
	h256 n;
...
void stopSealingAfterXBlocks(ele::Client* _c, unsigned _start, unsigned& io_mining)
{
	try
	{
		if (io_mining != ~(unsigned)0 && io_mining && asElhashClient(_c)->isMining() && _c->blockChain().details().number - _start == io_mining)
		{
			_c->stopSealing();
...
//	bool yesIReallyKnowWhatImDoing = false;
	...
	string jsonAdmin;
	ChainParams chainParams(genesisInfo(ele::Network::Frontier), genesisStateRoot(ele::Network::Frontier));
	...
	string clientName;
	string listenIP;
	unsigned short listenPort = 30707;
	string publicIP;
	string remoteHost;
	unsigned short remotePort = 30707;
...
//	double blockFees = 15.0;
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else if (arg == "--extra-data" && i + 1 < argc)
		{
			...
			chainParams = ChainParams(genesisInfo(ele::Network::Olympic));
		else if (arg == "--morden" || arg == "--testnet")
			chainParams = ChainParams(genesisInfo(ele::Network::Morden), genesisStateRoot(ele::Network::Morden));
		else if (arg == "--bob")
		{
...
/*		else if ((arg == "-B" || arg == "--block-fees") && i + 1 < argc)
		...
		else if ((arg == "-e" || arg == "--element-price") && i + 1 < argc)
		{
			try
			{
				elementPrice = stof(argv[++i]);
			}
			catch (...)
			{
				cerr << "Bad " << arg << " option: " << argv[i] << endl;
...
/*		else if ((arg == "-P" || arg == "--priority") && i + 1 < argc)
	...
	if (!configJSON.empty())
	{
		...
		catch (...)
		{
			...
			cerr << "sample: " << endl << genesisInfo(ele::Network::Olympic) << endl;
			return 0;
		}
	}
...
//		c_gasFloorTarget = gasFloor;
	...
	if (g_logVerbosity > 0)
		cout << EleGrayBold "cpp-elementrem, a C++ Elementrem client" EleReset << endl;
	...
	auto caps = useWhisper ? set<string>{"ele", "shh"} : set<string>{"ele"};
	...
	dev::WebThreeDirect web3(
		WebThreeDirect::composeClientVersion("ele"),
		getDataDir(),
		chainParams,
		withExisting,
	...
	if (!extraData.empty())
		web3.elementrem()->setExtraData(extraData);

	auto toNumber = [&](string const& s) -> unsigned {
		if (s == "latest")
			return web3.elementrem()->number();
		if (s.size() == 64 || (s.size() == 66 && s.substr(0, 2) == "0x"))
			return web3.elementrem()->blockChain().number(h256(s));
		try {
			return stol(s);
		}
		catch (...)
	...
	if (mode == OperationMode::Export)
	{
		...
		{
			bytes block = web3.elementrem()->blockChain().block(web3.elementrem()->blockChain().numberHash(i));
			switch (exportFormat)
			{
	...
	if (mode == OperationMode::Import)
	{
		...
		while (in.peek() != -1)
		{
			...
			switch (web3.elementrem()->queueBlock(block, safeImport))
			{
			...
			tuple<ImportRoute, bool, unsigned> r = web3.elementrem()->syncQueue(10);
			imported += get<2>(r);
			...
			if ((unsigned)e >= last + 10)
			{
				...
				auto d = e - last;
				cout << i << " more imported at " << (round(i * 10 / d) / 10) << " blocks/s. " << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.elementrem()->number() << ")" << endl;
				last = (unsigned)e;
				lastImported = imported;
//				cout << web3.elementrem()->blockQueueStatus() << endl;
			}
		}

		while (web3.elementrem()->blockQueue().items().first + web3.elementrem()->blockQueue().items().second > 0)
		{
			this_thread::sleep_for(chrono::seconds(1));
			web3.elementrem()->syncQueue(100000);
		}
		double e = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - t).count() / 1000.0;
		cout << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.elementrem()->number() << ")" << endl;
		return 0;
	}
		...
		if (keyManager.exists())
		{
			if (!keyManager.load(masterPassword))
			{
				...
				while (true)
				{
					...
					if (keyManager.load(masterPassword))
						break;
					cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " + getDataDir("elementrem") + "/keys.info" << endl;
				}
			}
		}
		else
...
//	std::shared_ptr<ele::BasicGasPricer> gasPricer = make_shared<ele::BasicGasPricer>(u256(double(element / 1000) / elementPrice), u256(blockFees * 1000));
	std::shared_ptr<ele::TrivialGasPricer> gasPricer = make_shared<ele::TrivialGasPricer>(askPrice, bidPrice);
	ele::Client* c = nodeMode == NodeMode::Full ? web3.elementrem() : nullptr;
	if (c)
	{
		c->setGasPricer(gasPricer);
		DEV_IGNORE_EXCEPTIONS(asElhashClient(c)->setShouldPrecomputeDAG(m.shouldPrecompute()));
	...
	if (testingMode)
		...
		authenticator = [&](TransactionSkeleton const& _t, bool isProxy) -> bool {
			...
			string r = getResponse(_t.userReadable(isProxy,
				...
				{
					h256 contractCodeHash = web3.elementrem()->postState().codeHash(_t.to);
	...
	if (jsonRPCURL > -1 || ipc)
	{
		...
		accountHolder.reset(new SimpleAccountHolder([&](){ return web3.elementrem(); }, getAccountPassword, keyManager, authenticator));
		auto eleFace = new rpc::Ele(*web3.elementrem(), *accountHolder.get());
		rpc::TestFace* testEle = nullptr;
		if (testingMode)
			testEle = new rpc::Test(*web3.elementrem());

		if (jsonRPCURL >= 0)
		{
			rpc::AdminEle* adminEle = nullptr;
			...
			if (adminViaHttp)
			{
				...
				adminEle = new rpc::AdminEle(*web3.elementrem(), *gasPricer.get(), keyManager, *sessionManager.get());
			...
			jsonrpcHttpServer.reset(new FullServer(
				eleFace, new rpc::LevelDB(), new rpc::Whisper(web3, {}),
				new rpc::Net(web3), new rpc::Web3(web3.clientVersion()), personal,
				adminEle, adminNet, adminUtils,
				new rpc::Debug(*web3.elementrem()),
				testEle
			));
		...
		if (ipc)
		{
			jsonrpcIpcServer.reset(new FullServer(
				eleFace, new rpc::LevelDB(), new rpc::Whisper(web3, {}), new rpc::Net(web3),
				new rpc::Web3(web3.clientVersion()), new rpc::Personal(keyManager, *accountHolder),
				new rpc::AdminEle(*web3.elementrem(), *gasPricer.get(), keyManager, *sessionManager.get()),
				new rpc::AdminNet(web3, *sessionManager.get()),
				new rpc::AdminUtils(*sessionManager.get()),
				new rpc::Debug(*web3.elementrem()),
				testEle
			));
			auto ipcConnector = new IpcServer("gele");
