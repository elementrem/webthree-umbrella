...
Json::Value Debug::traceBlock(Block const& _block, Json::Value const& _json)
{
	...
	for (unsigned k = 0; k < _block.pending().size(); k++)
	{
		...
		Executive e(s, _block, k, m_ele.blockChain());
...
Json::Value Debug::debug_traceTransaction(string const& _txHash, Json::Value const& _json)
{
	...
	try
	{
		LocalisedTransaction t = m_ele.localisedTransaction(h256(_txHash));
		Block block = m_ele.block(t.blockHash());
		State s(State::Null);
		ele::ExecutionResult er;
		Executive e(s, block, t.transactionIndex(), m_ele.blockChain());
		e.setResultRecipient(er);
...
{
	Json::Value ret;
	Block block = m_ele.block(h256(_blockHash));
...
{
	Json::Value ret;
	Block block = m_ele.block(blockHash(std::to_string(_blockNumber)));
	ret["structLogs"] = traceBlock(block, _json);
	return ret;
}
...
Json::Value Debug::debug_storageAt(string const& _blockHashOrNumber, int _txIndex, string const& _address)
{
	...
	Block block = m_ele.block(blockHash(_blockHashOrNumber));
